 ...
 * Customer Class
 *
 * The FFL Commerce customer class handles storage of the current customer's data, such as location.
 *
 * DISCLAIMER
 *
...
class fflcommerce_customer extends fflcommerce_Singleton {
	/** constructor */
	protected function __construct(){
		...
		if(!isset(fflcommerce_session::instance()->customer)){
	...
	public function update_signed_in_customer($user_login, $user){
		...
		fflcommerce_session::instance()->customer = array(
			'country' => $country,
			'state' => $state,
			'postcode' => $postcode,
	...
	private function set_default_customer() {
		...
		$state = fflcommerce_countries::get_default_customer_state();
	...
	public static function is_taxable($shippable){
		...
		$shop_country = fflcommerce_countries::get_base_country();
		$my_country = fflcommerce_tax::get_customer_country();

		if(fflcommerce_countries::is_eu_country($shop_country)){
			return fflcommerce_countries::is_eu_country($my_country);
		}
	...
	public static function is_customer_outside_base($shippable){
		...
		if($country){
			...
			if(fflcommerce_countries::country_has_states($country)){
				$shop_state = fflcommerce_countries::get_base_state();
			...
			} else if(fflcommerce_countries::is_eu_country($shop_country) && $shop_country != $country){
				// if both base country and shipping country are in the EU, then outside country base is true
				$outside = fflcommerce_countries::is_eu_country($country);
			}
		}
	...
	public static function get_shipping_country(){
		...
		return fflcommerce_countries::get_default_customer_country();
	}

	private static function get_customer_session($name){
		return isset(fflcommerce_session::instance()->customer[$name]) ? fflcommerce_session::instance()->customer[$name] : null;
	}
	...
	public static function get_shipping_state(){
		...
		return fflcommerce_countries::get_default_customer_state();
	}
	...
	protected static function is_valid_shipping_state($country, $state){
		if($country){
			...
			if($state && $has_states){
				return fflcommerce_countries::country_has_state($country, $state);
			} else if($has_states) {
				return false;
			}
	...
	private static function set_customer_session($name, $value){
		if(!is_array(fflcommerce_session::instance()->customer)){
			fflcommerce_session::instance()->customer = array();
		}
		$customer = fflcommerce_session::instance()->customer;
		$customer[$name] = $value;
		fflcommerce_session::instance()->customer = $customer;
	}
	...
	public function update_signed_out_customer(){
		unset(fflcommerce_session::instance()->customer);
		$this->set_default_customer();
	}
}
