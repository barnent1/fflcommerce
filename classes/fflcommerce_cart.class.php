...
/**
 * Cart Class
 * The FFL Commerce cart class stores cart data and active coupons as well as handling customer sessions and some cart related urls.
 ...
 */
class fflcommerce_cart extends FFLCommerce_Singleton
{
	...
	protected function __construct()
	{
		...
		if (isset(fflcommerce_session::instance()->coupons)) {
			self::$applied_coupons = fflcommerce_session::instance()->coupons;
		}

		self::$tax = new fflcommerce_tax(100); //initialize tax on the cart with divisor of 100
	...
	{
		self::$cart_contents = (array)fflcommerce_session::instance()->cart;
	}
	...
	private static function calculate_cart_total()
	{
		...
		if (!empty(self::$cart_contents)) {
			foreach (self::$cart_contents as $key => $values) {
				...
				if (!($_product instanceof fflcommerce_product)) {
				...
				if (self::get_options()->get('fflcommerce_calc_taxes') == 'yes') {
					if ($_product->is_taxable()) {
						$shippable = fflcommerce_shipping::is_enabled() && $_product->requires_shipping();

						self::$tax->set_is_shipable($shippable);

						$price_includes_tax =
							self::get_options()->get('fflcommerce_tax_after_coupon') == 'yes'
							&& $current_product_discount > 0
								? false
								: self::get_options()->get('fflcommerce_prices_include_tax') == 'yes';

						$product_discounted_price = self::get_options()->get('fflcommerce_tax_after_coupon') == 'yes'
						&& $current_product_discount > 0
						...
						if (self::get_options()->get('fflcommerce_prices_include_tax') == 'yes') {
							if (self::get_options()->get('fflcommerce_tax_after_coupon') == 'yes' && $discounted_item_price >= 0) {
	...
	private static function reset_totals()
	{
		...
		fflcommerce_shipping::reset_shipping();
	}
	...
	public static function empty_cart()
	{
		...
		self::$applied_coupons = array();
		self::reset_totals();
		unset(fflcommerce_session::instance()->cart);
		unset(fflcommerce_session::instance()->coupons);
	...
	private static function calculate_product_discounts_total($_product, $values)
	{
		...
		if (!empty(self::$applied_coupons)) {
			foreach (self::$applied_coupons as $code) {
				...
				$price = self::get_options()->get('fflcommerce_tax_after_coupon') == 'yes'
	...
	{
		return !empty(fflcommerce_cart::$applied_coupons);
	}
	...
	public static function add_to_cart($product_id, $quantity = 1, $variation_id = '', $variation = array())
	{
		...
		$cart_item_data = (array)apply_filters('fflcommerce_add_cart_item_data', array(), $product_id);
		...
		if ($this_post->post_type != 'product') {
			fflcommerce::add_error(__('You cannot add this item to your Cart as it does not appear to be a valid Product.', 'fflcommerce'));
		...
		if (empty($variation_id)) {
			...
			$product = new fflcommerce_product_variation($variation_id);
		}
		...
		if ($product->get_price() === '') {
			fflcommerce::add_error(__('You cannot add this product to your cart because its price is not yet announced', 'fflcommerce'));
		...
		if (!$product->has_enough_stock($quantity + $in_cart_qty)) {
			fflcommerce::add_error(sprintf(__('We are sorry. We do not have enough "%s" to fill your request.', 'fflcommerce'), $product->get_title()));

			if (self::get_options()->get('fflcommerce_show_stock') == 'yes') {
				if ($in_cart_qty > 0) {
					fflcommerce::add_error(sprintf(__('You have %d of them in your Cart and we have %d available at this time.', 'fflcommerce'), $in_cart_qty, $product->get_stock()));
				} else {
					fflcommerce::add_error(sprintf(__('There are only %d left in stock.', 'fflcommerce'), $product->get_stock()));
				}
			}
		...
		if ($cart_item_key) {
			...
			self::set_quantity($cart_item_key, apply_filters('fflcommerce_cart_item_quantity', $quantity, $product, $cart_item_key));
		} else {
			// otherwise add new item to the cart
			self::$cart_contents[$cart_id] = apply_filters('fflcommerce_add_cart_item', array(
				'data' => $product,
				...
				'tax' => 0,
				'discount' => 0,
				'price_includes_tax' => self::get_options()->get('fflcommerce_prices_include_tax')
			), $cart_item_data);
		}
				...
				foreach (self::$applied_coupons as $key => $code) {
					...
					if (JS_Coupons::is_valid_coupon_for_product($code, $_product)) {
						if ($coupon['type'] == 'fixed_product') {
							self::$discount_total = self::$discount_total - (apply_filters('fflcommerce_coupon_product_fixed_amount', $coupon['amount'], $coupon) * $_product['quantity']);
	...
	public static function is_valid_coupon($coupon)
	{
		...
		if (!$coupon) {
			fflcommerce::add_error(__('Coupon does not exist or is no longer valid!', 'fflcommerce'));
		...
		if (!empty($pay_methods) && !empty($payment_method) && !in_array($payment_method, $pay_methods)) {
			fflcommerce::add_error(sprintf(__("The coupon '%s' is invalid with that payment method!", 'fflcommerce'), $coupon['code']));
		...
		if (!empty($coupon['order_total_min']) || !empty($coupon['order_total_max'])) {
			/* Can't use the fflcommerce_cart::get_cart_subtotal() method as it's not ready at this point yet. */
			$subtotal = self::$cart_contents_total;

			$order_total_max = apply_filters('fflcommerce_coupon_order_total_max', $coupon['order_total_max'], $coupon);
			if (!empty($coupon['order_total_max']) && $subtotal > $order_total_max) {
				fflcommerce::add_error(sprintf(__('Your subtotal does not match the <strong>maximum</strong> order total requirements of %.2f for coupon "%s" and it has been removed.', 'fflcommerce'), $order_total_max, $coupon['code']));

				return false;
			}

			$order_total_min = apply_filters('fflcommerce_coupon_order_total_min', $coupon['order_total_min'], $coupon);
			if (!empty($coupon['order_total_min']) && $subtotal < $order_total_min) {
				fflcommerce::add_error(sprintf(__('Your subtotal does not match the <strong>minimum</strong> order total requirements of %.2f for coupon "%s" and it has been removed.', 'fflcommerce'), $order_total_min, $coupon['code']));
		...
		if (!fflcommerce_cart::has_valid_products_for_coupon($coupon)) {
			fflcommerce::add_error(__('No products in your cart match that coupon!', 'fflcommerce'));
	...
	public static function set_session()
	{
		...
		unset(fflcommerce_session::instance()->chosen_shipping_method_id);
		unset(fflcommerce::instance()->selected_rate_id); // calculable shipping

		fflcommerce_session::instance()->cart = apply_filters('fflcommerce_cart_set_session', self::$cart_contents);
	...
	public static function calculate_totals()
	{
		...
		if (self::needs_shipping()) {
			fflcommerce_shipping::calculate_shipping(self::$tax);
		} else {
			fflcommerce_shipping::reset_shipping();
		}

		self::$shipping_total = fflcommerce_shipping::get_total();
		$shipping_method = fflcommerce_shipping::get_chosen_method();
		...
		if (self::get_options()->get('fflcommerce_calc_taxes') == 'yes' && !self::$tax->get_total_shipping_tax_amount()) {
			foreach (self::get_applied_tax_classes() as $tax_class) {
				if (!self::is_not_compounded_tax($tax_class)) { //tax compounded
					$discount = (self::get_options()->get('fflcommerce_tax_after_coupon') == 'yes' ? self::$discount_total : 0);
		...
		if (self::get_options()->get('fflcommerce_calc_taxes') == 'yes'
			&& self::get_options()->get('fflcommerce_prices_include_tax') == 'no'
			&& self::get_options()->get('fflcommerce_tax_after_coupon') == 'no'
		) {
		...
		if (self::get_options()->get('fflcommerce_tax_after_coupon') == 'yes') {
			...
			if ($total_cart_discounts > 0) {
				...
				if($total_tax > 0){
					foreach (self::get_applied_tax_classes() as $tax_class) {
						...
						foreach (fflcommerce_cart::get_coupons() as $code) {
							$coupon = JS_Coupons::get_coupon($code);
							switch ($coupon['type']) {
								case 'fixed_cart':
			...
			if (self::get_options()->get('fflcommerce_prices_include_tax') == 'no') {
				self::$total += $temp;
			} else {
				self::$total = self::$cart_contents_total_ex_tax + self::$shipping_total + $temp;
			}
		} else { //  Taxes are applied before coupons, 'fflcommerce_tax_after_coupon' == 'no'
			if (self::get_options()->get('fflcommerce_prices_include_tax') == 'no') {
	...
	public static function get_applied_tax_classes()
	{
		...
		if (fflcommerce_tax::get_customer_country() == '') {
			return array();
		}
	...
	public static function get_cart_subtotal($for_display = true, $apply_discount_and_shipping = false, $order_exclude_tax = false)
	{
		...
		$tax_label = 0; // use with fflcommerce_price. 0 for no label, 1 for ex. tax, 2 for inc. tax
		...
		if (!empty(self::$applied_coupons)) {
			foreach (self::$applied_coupons as $code) {
				if ($coupon = JS_Coupons::get_coupon($code)) {
					...
					switch ($coupon['type']) {
						case 'fixed_cart' :
							$cart_discount += apply_filters('fflcommerce_coupon_cart_fixed_amount', $coupon['amount'], $coupon);
							break;
						case 'percent' :
	...
	{
		do_action('fflcommerce_calculate_totals');
		return self::get_options()->get('fflcommerce_prices_include_tax') == 'yes' ? self::$subtotal_ex_tax : self::$subtotal;
	}
	...
	/**
	 * @deprecated Use fflcommerce_cart::is_valid_coupon()
	 * @param $coupon_code
	 * @return bool
	 */
	...
	/**
	 * @deprecated Use fflcommerce_cart::has_valid_products_for_coupon()
	 * @param $coupon
	 * @return bool
	 */
	...
	public static function get_cart_url()
	{
		...
		if ($cart_page_id) {
			return apply_filters('fflcommerce_get_cart_url', get_permalink($cart_page_id));
		}
	...
	{
		$checkout_page_id = fflcommerce_get_page_id('checkout');
		if ($checkout_page_id) {
			...
			return apply_filters('fflcommerce_get_checkout_url', $url);
		}
	...
	{
		$shop_page_id = fflcommerce_get_page_id('shop_redirect');
		if ($shop_page_id) {
			return apply_filters('fflcommerce_get_shop_page_id', get_permalink($shop_page_id));
		}
	...
	{
		return (self::get_options()->get('fflcommerce_ship_to_billing_address_only') == 'yes');
	}
	...
	public static function check_cart_item_stock()
	{
		foreach (self::$cart_contents as $values) {
			...
			if (!$_product->is_in_stock() || ($_product->managing_stock() && !$_product->has_enough_stock($values['quantity']))) {
				$error = new WP_Error();
				$errormsg = self::get_options()->get('fflcommerce_show_stock') == 'yes'
					? sprintf(__('Sorry, we do not have enough "%s" in stock to fulfill your order. We only have %d available at this time. Please edit your cart and try again. We apologize for any inconvenience caused.', 'fflcommerce'), $_product->get_title(), $_product->get_stock())
					: sprintf(__('Sorry, we do not have enough "%s" in stock to fulfill your order. Please edit your cart and try again. We apologize for any inconvenience caused.', 'fflcommerce'), $_product->get_title());
	...
	{
		return ($for_display ? fflcommerce_price(self::$total) : number_format(self::$total, 2, '.', ''));
	}

	/** gets the cart contents total (after calculation) */
	public static function get_cart_total()
	{
		return fflcommerce_price(self::$cart_contents_total);
	}
	...
	public static function get_tax_for_display($tax_class)
	{
		...
		if ((fflcommerce_cart::get_tax_amount($tax_class, false) > 0 && fflcommerce_cart::get_tax_rate($tax_class) > 0)
			|| fflcommerce_cart::get_tax_rate($tax_class) !== false
		) {
			$return = self::$tax->get_tax_class_for_display($tax_class).' ('.(float)fflcommerce_cart::get_tax_rate($tax_class).'%) ';
	...
	public static function get_tax_amount($tax_class, $with_price = true)
	{
		...
		return ($with_price ? fflcommerce_price($tax) : number_format($tax, 2, '.', ''));
	}
	...
	{
		if (self::get_options()->get('fflcommerce_calc_taxes') != 'yes' || !fflcommerce_cart::get_total_discount()) {
			return false;
		}

		return (self::get_options()->get('fflcommerce_tax_after_coupon') == 'yes');
	}
	...
	public static function get_item_data($cart_item, $flat = false)
	{
		...
		// Variation data
		if ($cart_item['data'] instanceof fflcommerce_product_variation && is_array($cart_item['variation'])) {
			...
			foreach ($cart_item['variation'] as $name => $value) {
				...
				if (taxonomy_exists('pa_'.$name)) {
					...
					$name = $cart_item['data']->attribute_label('pa_'.$name);
					$value = apply_filters('fflcommerce_product_attribute_value_custom', $value, 'pa_'.$name);
				}
		...
		$other_data = apply_filters('fflcommerce_get_item_data', array(), $cart_item);
