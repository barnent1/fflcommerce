...
class fflcommerce_validation
{
	...
	public static function is_postcode($postcode, $country)
	{
		...
		if (FFLCommerce_Base::get_options()->get('fflcommerce_enable_postcode_validating') == 'yes') {
			switch ($country) {
				case 'GB':
					fflcommerce_log("VALIDATE POSTCODE: country = GB");
					return self::is_GB_postcode($postcode);
				default:
					$regex = '/^'.self::$postcodes[$country].'$/';
					fflcommerce_log("VALIDATE POSTCODE: country = ".$country." & regex = ".$regex);
