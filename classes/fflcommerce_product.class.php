 ...
 */
class fflcommerce_product extends FFLCommerce_Base
{

	// LEGACY
	private static $attribute_taxonomies = null;

	public $id;           // : fflcommerce_template_functions.php on line 99 // This is just an alias for $this->ID
	public $ID;
	public $exists;       // : fflcommerce_cart.class.php on line 66
	public $product_type; // : fflcommerce_template_functions.php on line 271
	public $sku;          // : fflcommerce_template_functions.php on line 246
	public $brand;
	public $gtin;
	public $mpn;

	public $data;         // fflcommerce_tax.class.php on line 186
	...
	protected $fflcommerce_options;
	private $weight;
	private $length; // : admin/fflcommerce-admin-post-types.php on line 168
	private $width;
	private $height;
	private $tax_status = 'taxable';
	private $tax_class;
	private $featured = false;         // : admin/fflcommerce-admin-post-types.php on line 180
	private $manage_stock = false;    // for managed stock only
	private $stock_status = 'instock'; // all sales whether managed stock or not
	private $backorders;
	private $quantity_sold; // : fflcommerce_template_functions.php on line 328
	private $attributes = array(); // : fflcommerce_product_variation.php uses as well
	...
	public function __construct($ID)
	{
		...
		// Define data
		$this->regular_price = isset($meta['regular_price'][0]) ? apply_filters('fflcommerce_get_regular_price', $meta['regular_price'][0], $this) : null;
		$this->sale_price = isset($meta['sale_price'][0]) ? apply_filters('fflcommerce_get_sale_price', $meta['sale_price'][0], $this) : null;
		...
		$this->sale_price = apply_filters('fflcommerce_sale_price', $this->sale_price, $this);
	...
	public static function getAttributeTaxonomies()
	{
		...
		if (self::$attribute_taxonomies === null) {
			self::$attribute_taxonomies = $wpdb->get_results("SELECT * FROM ".$wpdb->prefix."fflcommerce_attribute_taxonomies;");
		}
	...
	public function modify_stock($by)
	{
		...
		if (self::get_options()->get('fflcommerce_notify_no_stock_amount') >= 0
			&& self::get_options()->get('fflcommerce_notify_no_stock_amount') >= $this->stock
			&& self::get_options()->get('fflcommerce_hide_no_stock_product') == 'yes'
		) {
			update_post_meta($this->ID, 'visibility', 'hidden');
		} else if ($this->stock > self::get_options()->get('fflcommerce_notify_no_stock_amount')
	...
	public function managing_stock()
	{
		...
		if (self::get_options()->get('fflcommerce_manage_stock') != 'yes') {
			return false;
		}
	...
	public function requires_shipping()
	{
		...
		return apply_filters('fflcommerce_requires_shipping', (!($this->is_type(array(
			'downloadable',
			'virtual',
			'subscription'
		)))), $this->id);
	...
	{
		return apply_filters('fflcommerce_product_title', get_the_title($this->ID), $this);
	}
	...
	{
		return apply_filters('fflcommerce_product_link', get_permalink($this->ID), $this);
	}
	...
	public function get_children()
	{
		...
		$this->children = apply_filters('fflcommerce_get_product_children', $children, $this);
	...
	public function get_availability()
	{
		...
		if (self::get_options()->get('fflcommerce_manage_stock') != 'yes' || $this->is_type(array('grouped', 'variable'))) {
			return false;
		}

		// Start as in stock
		$notice = array(
			'availability' => __('In Stock', 'fflcommerce'),
			'class' => null,
		);
		...
		if ($this->is_in_stock()) {
			...
			if ($this->stock <= 0 && $this->backorders_allowed()) {
				$notice['availability'] = __('Available for order', 'fflcommerce');
			} else if (self::get_options()->get('fflcommerce_show_stock') == 'yes' && !$this->has_child() && $this->stock > 0) {
				// Check if we want user to get how many items is available
				$notice['availability'] .= ': '.$this->stock.' '.__(' available', 'fflcommerce');
			}
		} else {
			$notice['availability'] = __('Out of Stock', 'fflcommerce');
			$notice['class'] = 'out-of-stock';
		}

		return apply_filters('fflcommerce_product_availability', $notice, $this);
	}
	...
	public function is_in_stock($below_stock_threshold = false)
	{
		...
		if ($this->managing_stock() && ($below_stock_threshold ? $this->stock > self::get_options()->get('fflcommerce_notify_no_stock_amount') : $this->stock > 0)) {
			return true;
		}
	 ...
	 * @return  fflcommerce_product
	 */
	public function get_child($child_ID)
	{
		if ($this->is_type('variable')) {
			return new fflcommerce_product_variation($child_ID);
		}

		return new fflcommerce_product($child_ID);
	}
	...
	{
		if (self::get_options()->get('fflcommerce_calc_taxes') !== 'yes' || self::get_options()->get('fflcommerce_prices_include_tax') === 'yes') {
	...
	public function get_price()
	{
		...
		if ($this->is_on_sale()) {
			...
			$price = apply_filters('fflcommerce_product_get_regular_price', $this->regular_price, $this->ID);
		}

		return apply_filters('fflcommerce_product_get_price', $price, $this->ID);
	}
	...
	public function get_price_html()
	{
		...
		if ($this->is_type('variable') || $this->is_type('grouped')) {
			...
			if (!($children = $this->get_children())) {
				return __('Unavailable', 'fflcommerce');
			}
			...
			if (!$sameprice) {
				$html = '<span class="from">'._x('From:', 'price', 'fflcommerce').'</span> ';
				reset($array);
				$id = key($array);
				...
				if ($child->is_on_sale()) {
					...
					$html .= fflcommerce_price($child->get_price());
				}
				...
				$html = fflcommerce_price(reset($array));
			}

			if (empty($array)) {
				if (!$childInStock) {
					$html = __('Out of stock', 'fflcommerce');
				} else {
					$html = __('Price Not Announced', 'fflcommerce');
				}
			}

			return apply_filters('fflcommerce_product_get_price_html', $html, $this, $this->regular_price);
		}
		...
		if ($this->is_on_sale()) {
			...
			$html = fflcommerce_price($this->get_price());
		}

		if ($this->get_price() == 0) {
			$html = __('Free', 'fflcommerce');
		}
	...
	public function get_rating_html($location = '')
	{
		...
		$star_size = apply_filters('fflcommerce_star_rating_size'.$location, 16);
		...
		return '<div class="star-rating" title="'.sprintf(__('Rated %s out of 5', 'fflcommerce'), $average_rating).'"><span style="width:'.($average_rating * $star_size).'px"><span class="rating">'.$average_rating.'</span> '.__('out of 5', 'fflcommerce').'</span></div>';
	}
	...
	public function list_attributes()
	{
		...
		if (self::get_options()->get('fflcommerce_enable_weight') == 'yes' && $this->get_weight()) {
			$html .= '<tr><th>'.__('Weight', 'fflcommerce').'</th><td>'.$this->get_weight().self::get_options()
					->get('fflcommerce_weight_unit').'</td></tr>';
		}

		// Output dimensions if we have it
		if (self::get_options()->get('fflcommerce_enable_dimensions') == 'yes') {
			if ($this->get_length()) {
				$html .= '<tr><th>'.__('Length', 'fflcommerce').'</th><td>'.$this->get_length().self::get_options()
						->get('fflcommerce_dimension_unit').'</td></tr>';
			}
			if ($this->get_width()) {
				$html .= '<tr><th>'.__('Width', 'fflcommerce').'</th><td>'.$this->get_width().self::get_options()
						->get('fflcommerce_dimension_unit').'</td></tr>';
			}
			if ($this->get_height()) {
				$html .= '<tr><th>'.__('Height', 'fflcommerce').'</th><td>'.$this->get_height().self::get_options()
						->get('fflcommerce_dimension_unit').'</td></tr>';
			}
		}
		...
		foreach ($attributes as $attr) {
			...
			if ((bool)$attr['is_taxonomy']) {
				...
				if (is_wp_error($product_terms)) {
					fflcommerce_log("product::list_attributes() - Attribute for invalid taxonomy = ".$attr['name']);
					continue;
				}
				...
				$value = apply_filters('fflcommerce_product_attribute_value_taxonomy', implode(', ', $terms), $terms, $attr);
			} else {
				$value = apply_filters('fflcommerce_product_attribute_value_custom', wptexturize($attr['value']), $attr);
			}
	...
	{
		if (self::get_options()->get('fflcommerce_enable_weight') != 'yes') {
			return false;
		}
	...
	public function attribute_label($name)
	{
		...
		if (strstr($name, 'pa_')) {
			...
			$label = $wpdb->get_var($wpdb->prepare("SELECT attribute_label FROM ".$wpdb->prefix."fflcommerce_attribute_taxonomies WHERE attribute_name = %s;", $name));
			...
			// Discovered in FFL Commerce 1.7, this function can be incorrectly called from
			// 'fflcommerce_get_formatted_variation' as a static class method
			// make sure we have an instance to work with here for custom text attributes before calling $this
			if ($this instanceof fflcommerce_product) {
				$label = $name;
				$attributes = $this->get_attributes();
		...
		return apply_filters('fflcommerce_attribute_label', $label);
	}
	...
	function get_default_attributes()
	{
		...
		return apply_filters('fflcommerce_product_default_attributes', (array)maybe_unserialize($default), $this);
	}
}
