 ...
 * Coupons Class
 *
 * The FFL Commerce coupons class gets coupon data from storage
 *
 * DISCLAIMER
 *
...
class JS_Coupons extends FFLCommerce_Base {
	...
	public static function get_coupon_types() {
		$coupon_types = array(
			'fixed_cart'        => __('Cart Discount', 'fflcommerce'),
			'percent'           => __('Cart % Discount', 'fflcommerce'),
			'fixed_product'     => __('Product Discount', 'fflcommerce'),
			'percent_product'   => __('Product % Discount', 'fflcommerce')
		);
		return apply_filters( 'fflcommerce_coupon_discount_types', $coupon_types );
	}
	...
	public static function get_coupons() {
		if ( empty( self::$coupons ) ) {
			...
			if ( ! empty( $our_coupons )) foreach ( $our_coupons as $id => $coupon ) {
				...
				self::$coupons[$coupon->post_name] = apply_filters( 'fflcommerce_get_shop_coupon_data', $values, $coupon->post_name );
			}
		}
		return apply_filters( 'fflcommerce_coupons', self::$coupons );
	}
	...
	public static function remove_coupon( $code ) {
		...
		if ( !is_array( fflcommerce_session::instance()->coupons ) )
			return false;

		/* Loop to find the key of this coupon */
		foreach ( fflcommerce_session::instance()->coupons as $key => $coupon ) :

			if ( $code == $coupon ) {

				unset( fflcommerce_cart::$applied_coupons[$key] );
				$data = (array) fflcommerce_session::instance()->coupons;
				unset( $data[$key] );
				fflcommerce_session::instance()->coupons = $data;
...
/** Deprecated */
class fflcommerce_coupons extends JS_Coupons {
