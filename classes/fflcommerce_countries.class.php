 ...
 *
 * The FFL Commerce countries class stores country/state data.
 *
 * DISCLAIMER
 *
 ...
 */
class fflcommerce_countries extends FFLCommerce_Base {
	...
	public static function get_base_country(){
		$default = self::get_options()->get('fflcommerce_default_country');
	...
	public static function get_default_customer_country(){
		$default = self::get_options()->get('fflcommerce_default_country_for_customer');
		if($default == -1){
			return '';
		}
		$country = explode(':', $default);
	...
	public static function get_countries(){
		$countries = array_map(function($item){ return __($item, 'fflcommerce'); }, self::$countries);
	...
	public static function get_allowed_countries(){
		...
		if(self::get_options()->get('fflcommerce_allowed_countries') !== 'specific'){
			return $countries;
		}

		$specific_countries = self::get_options()->get('fflcommerce_specific_allowed_countries');
	...
	public static function get_country($country_code){
		return self::has_country($country_code) ? __(self::$countries[$country_code], 'fflcommerce') : '';
	}
	...
	public static function country_dropdown_options(
		...
		if($add_empty){
			$output .= '<option value="-1">'.__('None', 'fflcommerce').'</option>';
		}
				...
				if(($states = self::get_states($country_key))){
					...
					if($show_all){
						if(!is_array($selected_country) || !in_array($country_key, $selected_country)){
							...
							$output .= '>'.__('All of', 'fflcommerce').' '.$country_value.'</option>';
						}
					}
					...
					if(!$show_all && ($selected_state == '*' && $selected_country == $country_key)){
						...
						self::get_options()->set('fflcommerce_default_country', $country_key.':'.end($state_keys));
					}
				} else {
					...
					$output .= ' value="'.esc_attr($country_key).'">'.__($country_value, 'fflcommerce').'</option>';
				}
			}
		}
	...
	function base_country_notice() {
		echo '
			<div class="error">
				<p>'.__('Your <strong>Base Country / Region</strong> setting has been changed to the last state in the country. Please update accordingly', 'fflcommerce').'!</p>
				<p>' . sprintf(__('<a href="%s" target="_TOP">Read why</a> we did this for you.','fflcommerce'), "https://github.com/fflcommerce/fflcommerce/issues/545" ) . '</p>
			</div>
		';
	}
}
