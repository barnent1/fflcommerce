...
/**
 * Order Class
 * The FFL Commerce order class handles order data.
 * DISCLAIMER
 ...
 */
class fflcommerce_order extends FFLCommerce_Base
{
	public $_data = array();
	public $id;
	public $order_date;
	...
	public function fflcommerce_order($id = '')
	{
		if ($id > 0) {
			apply_filters('fflcommerce_get_order', $this->get_order($id), $id, $this);
		}
	...
	public function populate($result)
	{
		...
		// Formatted Addresses
		$country = ($this->billing_country && fflcommerce_countries::has_country($this->billing_country)) ? fflcommerce_countries::get_country($this->billing_country) : $this->billing_country;
		...
		if ($this->shipping_address_1) {
			$country = ($this->shipping_country && fflcommerce_countries::has_country($this->shipping_country)) ? fflcommerce_countries::get_country($this->shipping_country) : $this->shipping_country;
			$address = array_map('trim', array(
	...
	private function get_order_tax_array($key)
	{
		...
		return fflcommerce_tax::get_taxes_as_array($array_string, $divisor);
	}

	public static function get_order_statuses_and_names()
	{
		$order_types = array(
			'new' => __('New', 'fflcommerce'),
			...
			'failed' => __('Failed', 'fflcommerce'), /* can be set from PayPal, not currently shown anywhere -JAP- */
			'denied' => __('Denied', 'fflcommerce'), /* can be set from PayPal, not currently shown anywhere -JAP- */
			'expired' => __('Expired', 'fflcommerce'), /* can be set from PayPal, not currently shown anywhere -JAP- */
			'voided' => __('Voided', 'fflcommerce'), /* can be set from PayPal, not currently shown anywhere -JAP- */
		);

		return apply_filters('fflcommerce_filter_order_status_names', $order_types);
	}
	...
	{
		return apply_filters('fflcommerce_order_number', _x('#', 'hash before order number', 'fflcommerce').$this->id, $this);
	}
	...
	public function get_total_tax($with_currency = false, $with_price_options = true)
	{
		...
		if ($with_price_options) {
			return fflcommerce_price($order_tax, array('with_currency' => $with_currency));
		} else {
	...
	private function get_shipping_tax()
	{
		...
		if ($tax == 0 && isset(fflcommerce_session::instance()->chosen_shipping_method_id) && isset(fflcommerce_session::instance()->selected_rate_id)) {
			foreach($this->get_tax_classes() as $class) {
				if (isset($this->order_tax[$class][fflcommerce_session::instance()->chosen_shipping_method_id.fflcommerce_session::instance()->selected_rate_id])) {
					$tax += (float)$this->order_tax[$class][fflcommerce_session::instance()->chosen_shipping_method_id.fflcommerce_session::instance()->selected_rate_id];
				}
			}
		}
	...
	public function get_tax_amount($tax_class, $has_price = true)
	{
		...
		return ($has_price ? fflcommerce_price($tax_amount) : $tax_amount);
	}
	...
	{
		return (isset($this->order_total_prices_per_tax_class_ex_tax[$tax_class]) ? fflcommerce_price($this->order_total_prices_per_tax_class_ex_tax[$tax_class]) : fflcommerce_price(0));
	}

	/** Gets subtotal */
	public function get_subtotal_to_display()
	{
		$subtotal = fflcommerce_price($this->order_subtotal);
		if ($this->order_tax > 0 && self::get_options()->get('fflcommerce_calc_taxes') == 'yes') {
			$subtotal .= __(' <small>(ex. tax)</small>', 'fflcommerce');
		}
	...
	public function get_shipping_to_display($inc_tax = false)
	{
		if ($this->order_shipping > 0) {
			$shipping = fflcommerce_price($this->order_shipping);
			if ($this->order_shipping_tax > 0) { //tax applied to shipping
				// inc tax used with norway emails
				$shipping = ($inc_tax ? fflcommerce_price($this->order_shipping + $this->order_shipping_tax) : $shipping);
				$tax_tag = ($inc_tax ? __('(inc. tax)', 'fflcommerce') : __('(ex. tax)', 'fflcommerce'));
				$shipping .= sprintf(__(' <small>%s %s</small>', 'fflcommerce'), $tax_tag, ucwords($this->shipping_service));
			} else { // when no tax applied to shipping
				$shipping .= sprintf(__(' <small>%s</small>', 'fflcommerce'), ucwords($this->shipping_service));
			}
		} else {
			$shipping = __('Free!', 'fflcommerce');
		}
	...
	public function update_status($new_status_slug, $note = '')
	{
		...
		if ($new_status) {
			...
			if ($this->status != $new_status->slug) {
				...
				$this->add_order_note($note.' '.sprintf(__('Order status changed from %s to %s.', 'fflcommerce'), __($old_status->name, 'fflcommerce'), __($new_status->name, 'fflcommerce')));
				// Date
	...
	public function add_order_note($note, $private = 1)
	{
		$comment = array(
			...
			'comment_author' => __('FFL Commerce', 'fflcommerce'),
			'comment_author_email' => '',
			'comment_author_url' => '',
			'comment_content' => $note,
			'comment_type' => 'order_note',
			'comment_agent' => __('FFL Commerce', 'fflcommerce'),
			'comment_parent' => 0,
	...
	public function payment_complete()
	{
		...
		// Add the sale
		$this->add_sale();
		do_action('fflcommerce_payment_complete', $this->id);
	}
