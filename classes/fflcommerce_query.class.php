<?php
/**
 * FFL Commerce Catalog Query Class
 *
 * DISCLAIMER
 *
...
class fflcommerce_catalog_query extends FFLCommerce_Singleton {
	...
	protected function __construct() {
		...
		self::add_filter( 'request', 'fflcommerce_get_product_ids_in_view', 1 );
	...
	/**
	 * Alters the main wordpress query when on a FFL Commerce product listing.
	 *
	 ...
	 *
	 * The whole resulting request can be filtered using the 'fflcommerce-request' filter
	 *
	...
	public function catalog_query_filter( $request ) {
		...
		global $fflcommerce_all_post_ids_in_view;

		// we only work on FFL Commerce product lists
		if ( ! self::is_product_list() ) return $request;

		$request['post_status'] = 'publish';
		$request['posts_per_page'] = apply_filters( 'loop_shop_per_page', FFLCommerce_Base::get_options()->get( 'fflcommerce_catalog_per_page' ));
		...
		$request['post__in'] = apply_filters( 'loop-shop-posts-in', $fflcommerce_all_post_ids_in_view );

		return apply_filters( 'fflcommerce-request', $request );	/* give it back to WordPress for query_posts() */
	}
	...
	private function meta_query( $request ) {
		...
		if(FFLCommerce_Base::get_options()->get( 'fflcommerce_hide_no_stock_product' ) == 'yes'){
			$meta[] = array(
				'key'    => 'stock_status',
				'value'  => 'outofstock',
				'compare'=> '!='
	 ...
	 * @since 0.9.9
	 **/
	function fflcommerce_get_product_ids_in_view( $request ) {

		global $fflcommerce_all_post_ids_in_view;

		$fflcommerce_all_post_ids_in_view = array();
		...
		if ( $this_query->is_post_type_archive( 'product' )
			...
			$fflcommerce_all_post_ids_in_view = array_merge($fflcommerce_all_post_ids_in_view, $custom_query);

		endif;

		$fflcommerce_all_post_ids_in_view[] = 0;
