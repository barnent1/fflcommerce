...
/**
 * FFL Commerce Licence Validation Class used by downloadable digital products
 ...
 */
class fflcommerce_licence_validator
{
	...
	private $validator_token = 'fflcommerce-licence-validator';
	private $validator_prefix = 'fflcommerce_licence_validator_';
	...
	public function __construct($file, $identifier, $home_shop_url)
	{
		...
		$this->override_validation_url = get_option('fflcommerce_license_override_validation_url', false);
		$this->custom_validation_url = get_option('fflcommerce_license_custom_validation_url', '');
	...
	public function display_inactive_plugin_warning()
	{
		...
		<div class="error">
			<p>
				<?php printf(__('The License key for <i><b>%s</b></i> is not valid . Please enter your <b>Licence Key</b> on the FFL Commerce -> <a href="%s">Manage Licences</a> menu with your <b>Order email address</b>.  Until then, the plugin will not be enabled for use.', 'fflcommerce'),
					$this->title,
					admin_url('admin.php?page=fflcommerce-licence-validator'
				)); ?>
			</p>
		</div>
		<?php
	...
	/**
	 * Adding the Manage Licences menu to FFL Commerce Menu
	 *
	 * @return boolean
	 */
	public function register_nav_menu_link()
	{
		// Don't register the FFL Commerce Manage Licences submenu if it's already available.
		...
		add_submenu_page(
			'fflcommerce',
			__('Manage Licences', 'fflcommerce'),
			__('Manage Licences', 'fflcommerce'),
			'manage_fflcommerce',
	...
	/**
	 * Method checks whether FFL Commerce submenu option is already added by another extension
	 *
	 * @return boolean
	 */
	private function submenu_exists()
	{
		...
		if (isset($submenu['fflcommerce']) && is_array($submenu['fflcommerce'])) {
			foreach ($submenu['fflcommerce'] as $key => $value) {
	...
	/**
	 * Plugin Version and update Information for a FFL Commerce Licence API request
	 *
	...
	{
		// POST data to send to the FFL Commerce Licencing API
		$args = array(
			'email' => $email,
	...
	/**
	 * Prepare a request and send it to the FFL Commerce Licence API on the selling shop
	 *
	 * @param string $action
	 * @param array $args
	 * @return boolean
	...
	private function display_incorrect_update_warning()
	{
		...
		<div class="error">
			<p>
				<?php echo sprintf(__('The License key for <i><b>%s</b></i> is outdated. Please renew your license. Until then, the update for this plugin will not be accessible.', 'fflcommerce'), $this->title); ?>
			</p>
		</div>
	<?php
	}
		...
		if ($action == 'plugin_information') {
			if ($arg->slug === $this->file_slug) {
				...
				if (isset($response->sections)) {
					...
					$obj->sections = array('changelog' => __('No update information available.', 'fflcommerce'));
				}
	...
	function in_plugin_update_message($plugin_data, $r)
	{
		...
		$m = __('Please enter your license key on the <a href="%s">Manage Licences</a> page to enable automatic updates. You can buy your licence on <a href="%s">fflcommerce.com</a>.', 'fflcommerce');
		echo '<br />' . sprintf($m, admin_url('admin.php?page=fflcommerce-licence-validator'), $r->url);
	}

	/**
	 * Display FFL Commerce Manage Licences page.
	 *
	 * @return void
	 */
	public function admin_manage_licences()
	{
		...
		?>
		<div class="wrap">
			<h2><?php _e('Manage FFL Commerce Digital Plugin Licences', 'fflcommerce'); ?></h2>
			...
			<?php endforeach; ?>
			<p>
				<?php _e('To <em>activate</em> the licence, enter your licence keys and email addresses you used when you ordered the plugins.', 'fflcommerce'); ?>
				<?php _e('<br />To <em>de-activate</em> the licence, remove the licence key, but leave the email address.', 'fflcommerce'); ?>
			</p>
			...
			<form name="<?php echo $this->validator_token; ?>-login" id="<?php echo $this->validator_token; ?>-login"
				...
				<fieldset>
					<label for="override_validation_url"><?php _e('Override licensing validation URL for all extensions (by default it\'s defined in each extension)', 'fflcommerce') ?> <input id="override_validation_url" type="checkbox" name="override_validation_url" <?php if($this->override_validation_url) : echo 'checked="checked"'; endif; ?>></label>
					<div id="custom_url" style="display:none">
						<label for="custom_validation_url"><?php _e('Custom validation URL', 'fflcommerce'); ?> <input id="custom_validation_url" type="text" name="custom_validation_url" value="<?php echo $this->custom_validation_url; ?>" placeholder="http://www.fflcommerce.com/" style="width:338px;"> <?php _e('example: <i>http://fflcommerce.com/</i> or <i>http://www.fflcommerce.com/</i>', 'fflcommerce'); ?></label>
					</div>
					<script>
						jQuery(function($){
						...
						<?php foreach (self::$plugins as $plugin_identifier => $info) :
							...
							<tr>
								...
								<th scope="row" style="vertical-align: middle;">
									<label for="licence_key_email-<?php echo $plugin_identifier; ?>"><?php _e('Activation email', 'fflcommerce') ?></label>
								</th>
								<td>
								...
								<th>
									<?php if (!isset($keys[$plugin_identifier]['status']) || !$keys[$plugin_identifier]['status']) : ?>
										<b class="inactive-licence" style="color: #CC0000;"><?php _e('Licence is inactive!', 'fflcommerce'); ?></b>
									<?php endif; ?>
								</th>
							</tr>
						<?php endforeach; ?>
	...
	private function save_licence_keys()
	{
		...
		if(isset($_POST['override_validation_url'])) {
			update_option('fflcommerce_license_override_validation_url', $_POST['override_validation_url']);
			if(isset($_POST['custom_validation_url']) && preg_match('/^(http|https):\/\/([A-Z0-9][A-Z0-9_-]*(?:\.[A-Z0-9][A-Z0-9_-]*)+):?(\d+)?\//i', $_POST['custom_validation_url'])){
				update_option('fflcommerce_license_custom_validation_url', $_POST['custom_validation_url']);
			} else {
				$messages[] = array(
					'success' => false,
					'message' => __('<b>Invalid custom license URL.</b>', 'fflcommerce')
				);
			}
		} else {
			delete_option('fflcommerce_license_override_validation_url');
		}
		...
		foreach ($_POST['licence_keys'] as $product_id => $licence_key) {
			...
			if (empty($licence_key) && isset($keys[$product_id]['status']) && $keys[$product_id]['status'] && $licence_active) {
				...
				if ((isset($response->success) && $response->success) || (isset($response->code) && $response->code == 101)) {
					$messages[] = array(
						'success' => true,
						'message' => sprintf(__('<b>Key deactivated.</b> License key for <i>%s</i> has been <b>deactivated</b>.', 'fflcommerce'), self::$plugins[$product_id]['title'])
					);
					...
					$messages[] = array(
						'success' => false,
						'message' => sprintf(__('%s deactivation: ', 'fflcommerce'), self::$plugins[$product_id]['title']).$response->error
					);
				}
			} // Activate this key
			elseif (!$licence_active) {
				...
				if (isset($response->success) && $response->success) {
					$messages[] = array(
						'success' => true,
						'message' => sprintf(__('<b>Key activated.</b> License key for <i>%s</i> has been <b>activated</b>.', 'fflcommerce'), self::$plugins[$product_id]['title'])
					);
					...
					$messages[] = array(
						'success' => false,
						'message' => sprintf(__('%s activation: ', 'fflcommerce'), self::$plugins[$product_id]['title']).$response->error
					);
				}
			}
		}
	...
	/**
	 * Deactivate FFL Commerce Licence API request
	 *
	 * @param string $product_id
	 * @param string $licence_key
	 * @param string $email
	...
	/**
	 * Activate FFL Commerce Licence API request
	 *
	 * @param string $product_id
	 * @param string $licence_key
	 * @param string $email
	...
	/**
	 * Check for a valid licence FFL Commerce Licence API request
	 *
	 * @param string $product_id
	 * @param string $licence_key
	 * @param string $email
