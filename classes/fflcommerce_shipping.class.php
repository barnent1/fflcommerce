 ...
 */
class fflcommerce_shipping extends FFLCommerce_Singleton
{
	...
	public static function shipping_inits()
	{
		...
		do_action('fflcommerce_shipping_init'); /* loaded plugins for shipping inits */

		$load_methods = apply_filters('fflcommerce_shipping_methods', array());
	...
	{
		return (self::is_enabled() && self::get_options()->get('fflcommerce_enable_shipping_calc') == 'yes' && fflcommerce_cart::needs_shipping());
	}
	 ...
	 *
	 * @param $tax fflcommerce_tax The tax
	 */
	public static function calculate_shipping($tax)
	{
		...
		if (self::$enabled == 'yes') :
			...
			if (!empty(fflcommerce_session::instance()->chosen_shipping_method_id)) :
				$chosen_method = fflcommerce_session::instance()->chosen_shipping_method_id;
			else :
				$chosen_method = '';
				$calc_cheapest = true;
			endif;
			...
			if (sizeof($_available_methods) > 0) :
				...
				if (is_numeric(fflcommerce_session::instance()->selected_rate_id) && !empty($chosen_method)) :
					...
					// current fflcommerce functionality
				...
				if ($chosen_method) :
					...
					self::$shipping_total = $_available_methods[$chosen_method]->get_selected_price(fflcommerce_session::instance()->selected_rate_id);
					self::$shipping_tax = $_available_methods[$chosen_method]->get_selected_tax(fflcommerce_session::instance()->selected_rate_id);
					self::$shipping_label = $_available_methods[$chosen_method]->get_selected_service(fflcommerce_session::instance()->selected_rate_id);
	 ...
	 * @param type $tax fflcommerce_tax class instance
