 ...
 */
class fflcommerce_tax extends FFLCommerce_Base {
	public $rates;
...
//            $tax_amount['fflcommerce_custom_rate']['amount'] = ($divisor > 0 ? ($total_tax - $shipping_tax) * $divisor : $total_tax - $shipping_tax);
			...
			$tax_amount['fflcommerce_custom_rate']['amount'] = ($divisor > 0 ? $total_tax * $divisor : $total_tax);
			$tax_rate = (empty($price_ex_tax) ? 0 : $total_tax / $price_ex_tax) * 100;
			$tax_amount['fflcommerce_custom_rate']['rate'] = number_format($tax_rate, 4, '.', '');
			$tax_amount['fflcommerce_custom_rate']['compound'] = false;
			$tax_amount['fflcommerce_custom_rate']['display'] = __('Tax', 'fflcommerce');
			$tax_amount['fflcommerce_custom_rate']['shipping'] = ($divisor > 0 ? $shipping_tax * $divisor : $shipping_tax);
	...
	public function get_total_tax_rate($total_item_price = 0){
		...
		if($total_item_price > 0){
			$tot_tax_rate = (self::get_options()
				->get('fflcommerce_prices_include_tax') == 'yes' ? round($total_tax / ($total_item_price - $total_tax) * 100, 2) : round($total_tax / $total_item_price * 100, 2));
		}
	...
	function get_tax_classes(){
		$classes = self::get_options()->get('fflcommerce_tax_classes');
		$classes = explode("\n", $classes);
		$classes_array = array();
	...
	function get_tax_rates(){
		$tax_rates = self::get_options()->get('fflcommerce_tax_rates');
	...
	{
		if (FFLCommerce_Base::get_options()->get('fflcommerce_country_base_tax') == 'shipping_country') {
			return fflcommerce_customer::get_shipping_country();
		}

		return fflcommerce_customer::get_country();
	}
	...
	{
		if (FFLCommerce_Base::get_options()->get('fflcommerce_country_base_tax') == 'shipping_country') {
			return fflcommerce_customer::get_shipping_state();
		}

		return fflcommerce_customer::get_state();
	}
	...
	public function get_tax_classes_for_customer(){
		...
		if(fflcommerce_session::instance()->chosen_shipping_method_id == 'local_pickup'){
		...
		if($allowed_countries === 'specific'){
			...
			if(fflcommerce_countries::country_has_states($country) && !in_array($state, array_keys(fflcommerce_countries::get_states($country)))){
				if(isset($this->rates[$country])){
					$states = array_keys($this->rates[$country]);
					$state = array_shift($states);
				} else {
					$state = fflcommerce_countries::get_base_state();
				}
			}
		}

		$state = ($state && fflcommerce_countries::country_has_states($country) ? $state : '*');
	...
	public function get_tax_classes_for_base(){
		$country = fflcommerce_countries::get_base_country();
		$state = fflcommerce_countries::get_base_state();
		$state = ($state && fflcommerce_countries::country_has_states($country) ? $state : '*');
		$tax_classes = (isset($this->rates[$country]) && isset($this->rates[$country][$state]) ? $this->rates[$country][$state] : false);
	...
	private function get_online_label_for_base($class = '*'){
		$country = fflcommerce_countries::get_base_country();
		$state = fflcommerce_countries::get_base_state();
		$state = (fflcommerce_countries::country_has_states($country) && $state ? $state : '*');

		return (isset($this->rates[$country]) && isset($this->rates[$country][$state]) ? $this->rates[$country][$state][$class]['label'] : __('Tax', 'fflcommerce'));
	}
			...
			foreach($this->get_applied_tax_classes() as $tax_class){
				if(isset($this->tax_amounts[$tax_class]['amount']) && isset($this->tax_amounts[$tax_class]['compound'])){
					if(($compounded && $this->tax_amounts[$tax_class]['compound'] == 'yes') || (!$compounded && $this->tax_amounts[$tax_class]['compound'] != 'yes')){
						...
						if($inc_shipping && isset($this->tax_amounts[$tax_class][fflcommerce_session::instance()->chosen_shipping_method_id.fflcommerce_session::instance()->selected_rate_id])){
							$tax_amount += round($this->tax_amounts[$tax_class][fflcommerce_session::instance()->chosen_shipping_method_id.fflcommerce_session::instance()->selected_rate_id], 2);
						}
					}
				}
			}
	...
	public function get_total_shipping_tax_amount(){
		...
		if(!empty($this->tax_amounts)){
			foreach($this->get_applied_tax_classes() as $tax_class){
				if(isset($this->tax_amounts[$tax_class][fflcommerce_session::instance()->chosen_shipping_method_id.fflcommerce_session::instance()->selected_rate_id])){
					$tax_amount += round($this->tax_amounts[$tax_class][fflcommerce_session::instance()->chosen_shipping_method_id.fflcommerce_session::instance()->selected_rate_id], 2);
				}
			}
		}
	...
	public function get_non_compounded_tax_amount($inc_shipping = true){
		...
		//TODO: number_format... might need to change this because of fflcommerce options available for formatting numbers on cart
		return ($this->tax_divisor > 0 ? number_format($tax_amount / $this->tax_divisor, 2, '.', '') : number_format($tax_amount, 2, '.', ''));
	}
		...
		if($this->get_tax_classes_for_customer()){
			...
			foreach($customer_tax_classes as $tax_class){
				...
				if(!$this->is_compound_tax($rate)){
					...
					if($this->has_tax($tax_class)){
						...
						$tax_amount[$tax_class]['display'] = ($this->get_online_label_for_customer($tax_class) ? $this->get_online_label_for_customer($tax_class) : __('Tax', 'fflcommerce'));
						$tax_classes_applied[] = $tax_class;
					}
			...
			if(!empty($tax_classes)){
				foreach($tax_classes as $tax_class){
					...
					$tax_amount[$tax_class]['compound'] = false;
					$tax_amount[$tax_class]['display'] = ($this->get_online_label_for_base($tax_class) ? $this->get_online_label_for_base($tax_class) : __('Tax', 'fflcommerce'));
					$tax_classes_applied[] = $tax_class;
				}
			} else {
				// auto calculate zero rate for all other countries outside of tax base
				$tax_amount['fflcommerce_zero_rate']['amount'] = 0;
				$tax_amount['fflcommerce_zero_rate']['rate'] = 0;
				$tax_amount['fflcommerce_zero_rate']['compound'] = false;
				$tax_amount['fflcommerce_zero_rate']['display'] = __('Tax', 'fflcommerce');
				$tax_classes_applied[] = 'fflcommerce_zero_rate';
			}
		}
	...
	public static function calculate_total_tax_rate($product_rates_array){
		...
		$tax_rate = null;
		if($product_rates_array && is_array($product_rates_array) && self::get_options()->get('fflcommerce_calc_taxes') == 'yes' && !empty($product_rates_array)){
			$tax_rate = 0;

			foreach($product_rates_array as $tax_class => $value){
				if(fflcommerce_product::get_non_compounded_tax($tax_class, $product_rates_array)){
					$tax_rate += round(fflcommerce_product::get_product_tax_rate($tax_class, $product_rates_array), 4);
				} else {
					$tax_rate += round((100 + $tax_rate) * (fflcommerce_product::get_product_tax_rate($tax_class, $product_rates_array) / 100), 4);
				}
			}
		}
	...
	public function update_tax_amount($tax_class, $amount, $recalculate_tax = true, $overwrite = false){
		if($tax_class){
			if(empty($this->tax_amounts)){
				...
				$this->tax_amounts[$tax_class]['display'] = ($this->get_online_label_for_customer($tax_class) ? $this->get_online_label_for_customer($tax_class) : __('Tax', 'fflcommerce'));
				$this->tax_amounts[$tax_class]['compound'] = false;
			}

			if($recalculate_tax){
				$rate = $this->get_rate($tax_class);
				$tax = $this->calc_tax($amount, $rate, ($this->is_compound_tax() ? false : self::get_options()->get('fflcommerce_prices_include_tax') == 'yes'));
	...
	public function get_tax_class_for_display($tax_class){
		return (!empty($this->tax_amounts[$tax_class]['display']) ? $this->tax_amounts[$tax_class]['display'] : __('Tax', 'fflcommerce'));
	}
	...
	public function get_rate($tax_class = '*', $rate_only = true){
		...
		$has_states = fflcommerce_countries::country_has_states($country);
	...
	private function get_shipping_tax_rates($tax_classes = array()){
		$rates = array();
		$country = fflcommerce_customer::get_shipping_country();
		$state = fflcommerce_customer::get_shipping_state();
	...
	public function calc_tax($price, $rate, $price_includes_tax = true){
		...
		//TODO: number_format... may need to change this because of fflcommerce options with number formatting
	...
	public function calculate_shipping_tax($price, $shipping_method_id, $tax_classes = array()){
		...
		$selected_rate_id = fflcommerce_session::instance()->selected_rate_id;
		...
		if(!empty($rates)){
			...
			if(!empty($tax_classes)){
				...
				$this->tax_amounts['fflcommerce_zero_rate'][$shipping_method_id.$selected_rate_id] = 0;
			}
		}
	...
	{
		if (isset($this->tax_amounts[$tax_class]) && isset($this->tax_amounts[$tax_class][fflcommerce_session::instance()->chosen_shipping_method_id.fflcommerce_session::instance()->selected_rate_id])) {
			return $this->tax_amounts[$tax_class][fflcommerce_session::instance()->chosen_shipping_method_id.fflcommerce_session::instance()->selected_rate_id]/100;
		}
