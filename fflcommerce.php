...
if ($memory_limit < FFLCOMMERCE_REQUIRED_WP_MEMORY * 1024 * 1024) {
    ...
    function fflcommerece_required_wp_memory_warning() {
        echo '<div class="error"><p>' .
        sprintf(__('<strong>Warning!</strong> FFL Commerce requires at least %sM of memory for WordPress! Your system currently has: %s. <a href="%s" target="_blank">How to change?</a>', 'fflcommerce'), FFLCOMMERCE_REQUIRED_MEMORY, WP_MEMORY_LIMIT, 'http://codex.wordpress.org/Editing_wp-config.php#Increasing_memory_allocated_to_PHP') .
        '</p></div>';
    }

    add_action('admin_notices', 'fflcommerece_required_wp_memory_warning');
}
...
function fflcommerce_admin_footer($text) {
    ...
    return sprintf(
            '<a target="_blank" href="https://www.fflcommerce.com/support/">%s</a> | %s', __('Contact support', 'fflcommerce'), str_replace(
                    array('[stars]', '[link]', '[/link]'), array(
...
function fflcommerce_set_image_sizes() {
    ...
    foreach ($sizes as $size => $altSize) {
        add_image_size(
                $size, $options->get('fflcommerce_' . $size . '_w'), $options->get('fflcommerce_' . $size . '_h'), ($options->get('fflcommerce_use_wordpress_' . $altSize . '_crop', 'no') == 'yes')
        );
    }
...
function fflcommerce_price($price, $args = array()) {
    ...
    $price = number_format(
            (double) $price, (int) $options->get('fflcommerce_price_num_decimals'), $options->get('fflcommerce_price_decimal_sep'), $options->get('fflcommerce_price_thousand_sep')
    );
                        ...
                        foreach ($post['terms'] as $term) {
                            ...
                            if (strstr($domain, 'pa_')) {
                                ...
                                if (!taxonomy_exists($domain)) {
                                    ...
                                    register_taxonomy($domain, array('product'), array(
                                        'hierarchical' => true,
                                        'labels' => array(
                                            'name' => $nicename,
