...
jQuery(function($){
	...
	$(':input.fflcommerce-enhanced-select').filter(':not(.enhanced)').each(function(){
		$(this).select2({
	...
	$(':input.fflcommerce-product-search').filter(':not(.enhanced)').each(function(){
		var select2_args = {
			allowClear: $(this).data('allow_clear') ? true : false,
			placeholder: $(this).data('placeholder'),
			...
			ajax: {
				...
				data: function(term, page){
					return {
						term: term,
						action: $(this).data('action') || 'fflcommerce_json_search_products_and_variations',
						security: fflcommerce_params.search_products_nonce
					};
				},
