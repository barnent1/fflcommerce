(function($) {
	// @todo: varmeta is a temporary name.. this should be a global fflcommerce var
	...
	$(function() {
		...
		setTimeout(function(){
			$('.fflcommerce_variation.postbox h3').unbind('click.postboxes');
		}, 100);
		...
		$('#variable_product_options')
			...
			.on('click', '.add_variation', function(e){
				...
				$(html).removeClass('closed').hide().prependTo('.fflcommerce_variations').slideDown(150);
				$(this).trigger('fflcommerce_add_variation');
			})
			...
			.on('click', '.upload_image_button', function(e){
				...
				if($field.val() != ''){
					$field.val('');
					$img.attr('src', fflcommerce_params.assets_url + '/assets/images/placeholder.png');
				} else {
			...
			.on({
				remove_all: function(e){
					if(!confirm(varmeta.i18n.remove_all))
						return;
					$('.fflcommerce_variation').each(function(){
