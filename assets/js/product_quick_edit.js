(function($) {
	...
	inlineEditPost.edit = function( id ) {
		...
		if ( $post_id > 0 ) {
			...
			var data = {
				action: 	'fflcommerce_get_product_stock_price',
				security: 	fflcommerce_quick_edit_params.get_stock_price_nonce,
				post_id:	$post_id
			};

			$.ajax( {
				type: 		'GET',
				url: 		fflcommerce_quick_edit_params.ajax_url,
				data: 		data,
				dataType:	'json',
	...
	$( 'body' ).on( 'click', '#bulk_edit', function() {
		...
		var data = {
			action: 		'fflcommerce_save_bulk_edit',
			security: 		fflcommerce_quick_edit_params.update_stock_price_nonce,
			post_ids:		$post_ids,
			stock:			$stock,
			price:			$price,
		};
