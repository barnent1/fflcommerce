jQuery(function($) {
	function fflcommerce_start_tabs() {
		var $tabs = $('.tabs');
	...
	function fflcommerce_stock_options() {
	...
	function fflcommerce_product_type_options() {
	...
	function fflcommerce_sale_picker() {
		// Sale price schedule
	...
	function fflcommerce_sortables() {
		$('.fflcommerce_attributes_wrapper').sortable({
			items:'.attribute',
	...
	function row_indexes() {
		$('.fflcommerce_attributes_wrapper .attribute').each(function(index, el) {
				$('.attribute_position', el).val(
					parseInt( $(el).index('.fflcommerce_attributes_wrapper .attribute') )
				);
		});
	}

	function fflcommerce_orders() {

		$(document.body).on('click', '#order_items_list button.remove_row', function(e) {
			e.preventDefault();
			var answer = confirm(fflcommerce_params.remove_item_notice);
			if (answer){
		...
		$(document.body).on('click', 'button.calc_totals', function(e) {
			e.preventDefault();
			var answer = confirm( fflcommerce_params.cart_total );
			if ( answer ){
		...
		$(document.body).on('click', 'button.billing-same-as-shipping', function(e){
			e.preventDefault();
			var answer = confirm(fflcommerce_params.copy_billing);
			if (answer){
		...
		$('button.add_shop_order_item').click(function(e) {
			...
			if (item_id) {
				$('table.fflcommerce_order_items').block({ message: null, overlayCSS: { background: '#fff url(' + fflcommerce_params.assets_url + '/assets/images/ajax-loader.gif) no-repeat center', opacity: 0.6 } });

				var data = {
					action: 		  'fflcommerce_add_order_item',
					item_to_add: 	item_id,
					item_no:      $('#order_items_list').children.length,
					security: 		fflcommerce_params.add_order_item_nonce
				};

				$.post( fflcommerce_params.ajax_url, data, function(response) {

					$('table.fflcommerce_order_items tbody#order_items_list').append( response );
					$('table.fflcommerce_order_items').unblock();
	...
	function fflcommerce_attributes() {
		// Initial Ordering
		var fflcommerce_attributes_table_items = $('.fflcommerce_attributes_wrapper').children('.attribute').get();
		fflcommerce_attributes_table_items.sort(function(a, b) {
		   var compA = Number($(a).attr('rel'));
		   var compB = Number($(b).attr('rel'));
		   return (compA < compB) ? -1 : (compA > compB) ? 1 : 0;
		})
		$(fflcommerce_attributes_table_items).each( function(idx, itm) { $('.fflcommerce_attributes_wrapper').append(itm); } );
		...
		$(document.body).on('click', 'button.hide_row', function(e) {
			e.preventDefault();
			var answer = confirm(fflcommerce_params.confirm_remove_attr)
			if (answer){
				$parent = $(this).parent();
			...
			if (!attribute) {
				...
				var $custom_panel = $('\
					<div class="postbox attribute custom">\
						...
						<h3 class="handle">'+fflcommerce_params.custom_attr_heading+'</h3>\
\
...
\
				...
				$custom_panel.hide().prependTo('.fflcommerce_attributes_wrapper').slideDown( 150, function() {
					$(this).find('.attribute-name').focus()
				});

			} else {

				// var size = $('table.fflcommerce_attributes tbody tr').size();
				// Reveal taxonomy row
				...
				$('.fflcommerce_attributes_wrapper').prepend( thisrow );
	...
	function fflcommerce_file_upload() {
		$('.upload_file_button').on('click', function(e){
			e.preventDefault();
			var $this = $(this);
			var $field = $this.prev();
			if(!this.bound){
				$this.fflcommerce_media({
					field: false,
					bind: false,
			...
			$(this).trigger('fflcommerce_media');
		});
	}
	...
	if ($product_type.length) {
		...
		fflcommerce_product_type_options();
		fflcommerce_sale_picker();
		fflcommerce_stock_options();
		fflcommerce_sortables();
	}

	fflcommerce_start_tabs();
	fflcommerce_orders();
	fflcommerce_attributes();
	fflcommerce_file_upload();
});
