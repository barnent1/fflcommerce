 ...
 */
class fflcommerce_shipping_method
{
	var $id;
	var $title;
	var $availability;
	...
	{
		FFLCommerce_Base::get_options()->install_external_options_onto_tab(__('Shipping', 'fflcommerce'), $this->get_default_options());
	}

	/**
	 * Default Option settings for WordPress Settings API using an implementation of the FFLCommerce_Options_Interface
	...
	{
		if($this->is_chosen()){
			if (is_numeric(fflcommerce_session::instance()->selected_rate_id)) {
				return $rate_index == fflcommerce_session::instance()->selected_rate_id;
			} else {
	...
	protected function get_ship_to_countries()
	{
		...
		} else if (FFLCommerce_Base::get_options()->get('fflcommerce_allowed_countries') == 'specific') {
			$ship_to_countries = FFLCommerce_Base::get_options()->get('fflcommerce_specific_allowed_countries');
		}
	 ...
	 *
	 * @param $tax fflcommerce_tax instance
	 */
	...
	// do not call this method from shipping plugins. FFL Commerce core handles this

	public function choose()
	{
		$this->chosen = true;
		fflcommerce_session::instance()->chosen_shipping_method_id = $this->id;
	}
		...
		if (!empty($selected_service)) {
			for ($i = 0; $i < $this->get_rates_amount(); $i++) {
				if ($this->get_selected_service($i) == $selected_service) {
					fflcommerce_session::instance()->selected_rate_id = $i;
					break;
				}
			}
		}
	...
	public function get_selected_service($rate_index)
	{
		...
		if ($this->title && $my_rate['service'] != $this->title) {
			$service = $my_rate['service'].__(' via ', 'fflcommerce').$this->title;
		} else {
	...
	public function get_cheapest_service()
	{
		...
		if ($this->title && $my_cheapest_rate['service'] != $this->title) {
			$service = $my_cheapest_rate['service'].__(' via ', 'fflcommerce').$this->title;
		} else {
	...
	public function get_cheapest_price()
	{
		...
		return apply_filters('fflcommerce_shipping_total_price', ($my_cheapest_rate == null ? $this->shipping_total : $my_cheapest_rate['price']));
	}
	...
	public function get_selected_price($rate_index)
	{
		...
		return apply_filters('fflcommerce_shipping_total_price', ($my_rate == null ? $this->shipping_total : $my_rate['price']));
	}
	...
	public function get_selected_tax($rate_index)
	{
		...
		return apply_filters('fflcommerce_shipping_tax_price', ($my_rate == null ? $this->shipping_tax : $my_rate['tax']));
	}
	...
	protected function get_cheapest_price_tax()
	{
		...
		return apply_filters('fflcommerce_shipping_tax_price', ($my_cheapest_rate == null ? $this->shipping_tax : $my_cheapest_rate['tax']));
	}
