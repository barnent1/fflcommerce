...
function get_fflcommerce_order_tracking ($atts) {
	return fflcommerce_shortcode_wrapper('fflcommerce_order_tracking', $atts);
}
	...
	if ($_POST) :
		...
		elseif ($order->id && $order_email && $order->get_order( apply_filters( 'fflcommerce_shortcode_order_tracking_order_id', $order->id ) )) :
			...
			if ($order->billing_email == $order_email) :
				...
				?>
				<?php do_action('fflcommerce_before_track_order_details', $order->id);?>
				<h2><?php _e('Order Details', 'fflcommerce'); ?></h2>
				<table class="shop_table">
					<thead>
						<tr>
							<th><?php _e('ID/SKU', 'fflcommerce'); ?></th>
							<th><?php _e('Title', 'fflcommerce'); ?></th>
							<th><?php _e('Price', 'fflcommerce'); ?></th>
							<th><?php _e('Quantity', 'fflcommerce'); ?></th>
						</tr>
					</thead>
					<tfoot>
                        <tr>
                            <?php if (($fflcommerce_options->get('fflcommerce_calc_taxes') == 'yes' && $order->has_compound_tax())
                                ...
                                <td colspan="3"><?php _e('Retail Price', 'fflcommerce'); ?></td>
                            <?php else : ?>
                                <td colspan="3"><?php _e('Subtotal', 'fflcommerce'); ?></td>
                            <?php endif; ?>
						...
						<?php if ($fflcommerce_options->get('fflcommerce_tax_after_coupon') == 'no' && $order->order_discount>0) : ?><tr class="discount">
							<td colspan="3"><?php _e('Discount', 'fflcommerce'); ?></td>
							<td>-<?php echo fflcommerce_price($order->order_discount); ?></td>
						</tr><?php endif; ?>
						<tr>
				...
				<?php do_action('fflcommerce_after_track_order_details', $order->id);?>

				<div style="width: 49%; float:left;">
					<h2><?php _e('Billing Address', 'fflcommerce'); ?></h2>
					<p><?php
				...
				<div style="width: 49%; float:right;">
					<h2><?php _e('Shipping Address', 'fflcommerce'); ?></h2>
					<p><?php
		...
		<form action="<?php echo esc_url( get_permalink($post->ID) ); ?>" method="post" class="track_order">
			...
			<p><?php _e('To track your order please enter your Order ID and email address in the boxes below and press return. This was given to you on your receipt and in the confirmation email you should have received.', 'fflcommerce'); ?></p>

			<p class="form-row form-row-first"><label for="orderid"><?php _e('Order ID', 'fflcommerce'); ?></label> <input class="input-text" type="text" name="orderid" id="orderid" placeholder="<?php _e('Found in your order confirmation email.', 'fflcommerce'); ?>" /></p>
			<p class="form-row form-row-last"><label for="order_email"><?php _e('Billing Email', 'fflcommerce'); ?></label> <input class="input-text" type="text" name="order_email" id="order_email" placeholder="<?php _e('Email you used during checkout.', 'fflcommerce'); ?>" /></p>
			<div class="clear"></div>
			<p class="form-row"><input type="submit" class="button" name="track" value="<?php _e('Track"', 'fflcommerce'); ?>" /></p>
			<?php fflcommerce::nonce_field('order_tracking') ?>
		</form>
		<?php
