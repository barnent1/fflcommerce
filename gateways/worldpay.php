...
class fflcommerce_worldpay extends fflcommerce_payment_gateway
{
		...
		<script type="text/javascript">
			...
			jQuery(document).ready(function($){
				$(document.body).on('click', '.payment_methods input.input-radio', function(){
					var label = $('#payment_method_fflcommerce_worldpay').next();
					var image = $(label).find('img');
					if($(this).attr('ID') == 'payment_method_fflcommerce_worldpay'){
	...
	public function worldpay_notices()
	{
		...
		if ($this->md5_encrypt == 'yes' && empty($this->secret_word)) {
			echo '<div class="error"><p>'.__('The WorldPay gateway <strong>Use MD5 Signature</strong> setting is enabled, but you have not entered a <strong>Secret Word</strong>.  The WorldPay gateway will not be available on the Checkout until you resolve this.', 'fflcommerce').'</p></div>';
		}
	}
	...
	{
		echo '<p>'.__('Thank you for your order, please click the button below to pay with WorldPay.', 'fflcommerce').'</p>';
	...
	public function display_worldpay_redirect_form($order_id)
	{
		...
		return fflcommerce_render_result('gateways/worldpay', array(
			'url' => $worldpay_url,
	...
	private function validate_response_origins_and_passwords()
	{
		...
		if (strpos($header_host, 'worldpay.com') !== false) {
			...
			$error['validate_origin_error'] = sprintf(__('The Payment response came from IP: %s and Domain: %s -- and this does not appear to be a WorldPay domain.', 'fflcommerce'), $header_ip, $header_host);
			fflcommerce_log($error['validate_origin_error'], 'WorldPay Gateway');
		}

		if ($this->receive_err_log == 'yes' && !$validated) {
			$info = sprintf(__('Order #%s ', 'fflcommerce'), $this->get_post('cartId'));
	...
	private function email_worldpay_error_logs($error = array(), $posted = array(), $info = '')
	{
		...
		else $email = FFLCommerce_Base::get_options()->get('fflcommerce_email');
	...
	protected function get_default_options()
	{
		return array(
			...
			array(
				'name' => __('Enable WorldPay', 'fflcommerce'),
				'desc' => '',
				'tip' => '',
				'id' => 'fflcommerce_worldpay_is_enabled',
				'std' => 'no',
				'type' => 'checkbox',
				'choices' => array(
			...
			array(
				...
				'desc' => '',
				'tip' => __('This controls the title which the user sees during checkout and also appears as the Payment Method on final Orders.', 'fflcommerce'),
				'id' => 'fflcommerce_worldpay_method_title',
				'std' => __('Credit Card via WorldPay', 'fflcommerce'),
				'type' => 'text'
			),
			array(
				'name' => __('Description', 'fflcommerce'),
				'desc' => '',
				'tip' => __('This controls the description which the user sees during checkout.', 'fflcommerce'),
				'id' => 'fflcommerce_worldpay_checkout_description',
				'std' => __("When you Place your Order, you will be directed to the secured WorldPay servers to enter your credit card information.  (Your Billing Address above must match that used on your Credit Card)", 'fflcommerce'),
				'type' => 'textarea'
			),
			array(
				'name' => __('Enable WorldPay Test Mode', 'fflcommerce'),
				'desc' => 'Enable to make test transactions.',
				'tip' => '',
				'id' => 'fflcommerce_worldpay_test_mode',
				'std' => 'no',
				'type' => 'checkbox',
				'choices' => array(
					'no' => __('No', 'fflcommerce'),
			...
			array(
				'name' => __('Installation ID', 'fflcommerce'),
				'desc' => '',
				'tip' => __('Please enter your WorldPay Installation ID.', 'fflcommerce'),
				'id' => 'fflcommerce_worldpay_install_id',
				'std' => '',
				'type' => 'text'
			),
			array(
			...
			array(
				'name' => __('Secret Word', 'fflcommerce'),
				'desc' => '',
