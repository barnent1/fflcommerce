...
add_filter( 'fflcommerce_payment_gateways', 'add_skrill_gateway', 40 );


class skrill extends fflcommerce_payment_gateway {
	...
	protected function get_default_options() {
		...
		$defaults[] = array( 	
			...
			'type' => 'title', 
			'desc' => __('Skrill works by using an iFrame to submit payment information securely to Moneybookers.', 'fflcommerce') 
		);

		// List each option in order of appearance with details
		$defaults[] = array(
			'name'		=> __('Enable Skrill','fflcommerce'),
			'desc' 		=> '',
			'tip' 		=> '',
			'id' 		=> 'fflcommerce_skrill_enabled',
			'std' 		=> 'no',
			'type' 		=> 'checkbox',
			'choices'	=> array(
		...
		$defaults[] = array(
			...
			'id' 		=> 'fflcommerce_skrill_title',
			'std' 		=> __('Skrill','fflcommerce'),
			'type' 		=> 'text'
		);

		$defaults[] = array(
			'name'		=> __('Skrill merchant e-mail','fflcommerce'),
			'desc' 		=> '',
			'tip' 		=> __('Please enter your skrill email address; this is needed in order to take payment!','fflcommerce'),
			'id' 		=> 'fflcommerce_skrill_email',
			'std' 		=> '',
			'type' 		=> 'email'
		);

		$defaults[] = array(
			'name'		=> __('Skrill Secret Word','fflcommerce'),
			'desc' 		=> '',
			'tip' 		=> __('Please enter your skrill secretword; this is needed in order to take payment!','fflcommerce'),
			'id' 		=> 'fflcommerce_skrill_secret_word',
			'std' 		=> '',
			'type' 		=> 'text'
		);

		$defaults[] = array(
			'name'		=> __('Skrill Customer ID','fflcommerce'),
			'desc' 		=> '',
			'tip' 		=> __('Please enter your skrill Customer ID; this is needed in order to take payment!','fflcommerce'),
			'id' 		=> 'fflcommerce_skrill_customer_id',
			'std' 		=> '',
			'type' 		=> 'text'
		);

		$defaults[] = array (
			'name'      => __('Skrill payment icon','fflcommerce'),
			'desc'      => __('Use the full URL to the image including http://','fflcommerce'),
			'tip'		=> __('The URL to an icon image to display on the Checkout','fflcommerce'),
			'id'		=> 'fflcommerce_skrill_icon',
			'std' 		=> $this->icon,
			'type' 		=> 'text'
		);

		$defaults[] = array(
			'name'		=> __('Skrill payment methods','fflcommerce'),
			...
			'id' 			=> 'fflcommerce_skrill_payment_methods_multicheck',
			'type' 		=> 'multicheck',
			'std'       => 'ACC',
			'choices'	=> array(
				'ACC'           => __('All credit card types','fflcommerce'),
				'VSA'           => __('Visa','fflcommerce'),
				'MSC'           => __('MasterCard','fflcommerce'),
				'VSE'           => __('Visa Electron','fflcommerce')
			),
    ...
    public function generate_skrill_form() {
		...
		$skrill_args = array(
			...
			'hide_login'           => 1,
			'confirmation_note'    => __('Thank you for shopping','fflcommerce'),
			...
			'amount'               => $order_total,
			'currency'             => FFLCommerce_Base::get_options()->get_option('fflcommerce_currency'),
	...
	function receipt_skrill( $order ) {
		...
		echo '<p>'.__('Thank you for your order, please complete the secure (SSL) form below to pay with Skrill.', 'fflcommerce').'</p>';
	...
	function successful_request( $posted ) {
	    ...
	    if ( !empty($posted['mb_transaction_id']) ) {
			...
			$order = new fflcommerce_order( (int) $posted['transaction_id'] );

			if ($order->status !== 'completed') :
		        // We are here so lets check status and do actions
		        switch ($posted['status']) :
		            case '2' : // Processed
		                $order->add_order_note( __('Skrill payment completed', 'fflcommerce') );
		                $order->payment_complete();
		            break;
		            case '0' : // Pending
		            case '-2' : // Failed
		                $order->update_status('on-hold', sprintf(__('Skrill payment failed (%s)', 'fflcommerce'), strtolower($posted['status']) ) );
		            break;
		            case '-1' : // Cancelled
		            	$order->update_status('cancelled', __('Skrill payment cancelled', 'fflcommerce'));
		            break;
		            default:
		            	$order->update_status('cancelled', __('Skrill exception', 'fflcommerce'));
		            break;
		        endswitch;
			endif;
