...
class paypal extends fflcommerce_payment_gateway
{
	...
	public function admin_scripts()
	{
		...
		<script type="text/javascript">
			/*<![CDATA[*/
			jQuery(function($){
				$('input#fflcommerce_paypal_testmode').click(function(){
					if($(this).is(':checked')){
	...
	{
		echo '<p>'.__('Thank you for your order, please click the button below to pay with PayPal.', 'fflcommerce').'</p>';
	...
	public function generate_paypal_form($order_id)
	{
		...
		$paypal_args = array_merge(
			array(
				...
				'no_note' => 1,
				'currency_code' => FFLCommerce_Base::get_options()->get('fflcommerce_currency'),
				'charset' => 'UTF-8',
				'rm' => 2,
				'upload' => 1,
		...
		if (FFLCommerce_Base::get_options()->get('fflcommerce_prices_include_tax') == 'yes') {
			...
			foreach ($order->items as $item) {
				...
				if ($_product->exists() && $item['qty']) {
					...
					$paypal_args['amount_'.$item_loop] = number_format(apply_filters('fflcommerce_paypal_adjust_item_price', $item['cost'], $item, 10, 2), $this->decimals); //Apparently, Paypal did not like "28.4525" as the amount. Changing that to "28.45" fixed the issue.
				}
			}
		...
		return fflcommerce_render_result('gateways/paypal', array(
			'url' => $url,
			'fields' => $paypal_args,
		));
	}
	...
	function legacy_ipn_response()
	{
		if (!empty($_GET['paypalListener']) && $_GET['paypalListener'] == 'paypal_standard_IPN') {
			do_action('fflcommerce_api_js_gateway_paypal');
		}
	}
	...
	function check_ipn_request_is_valid()
	{
		...
		fflcommerce_log('Received invalid response from PayPal!');
		fflcommerce_log('IPN Response: '.print_r($response, true));

		if (is_wp_error($response)) {
			fflcommerce_log('PayPal IPN WordPress Error message: '.$response->get_error_message());
		}
	...
	protected function get_default_options()
	{
		return array(
			...
			array(
				'name' => __('Enable PayPal Standard', 'fflcommerce'),
				'desc' => '',
				'tip' => '',
				'id' => 'fflcommerce_paypal_enabled',
				'std' => 'yes',
				'type' => 'checkbox',
				'choices' => array(
			...
			array(
				...
				'id' => 'fflcommerce_paypal_title',
				'std' => __('PayPal', 'fflcommerce'),
				'type' => 'text'
			),
			array(
				...
				'id' => 'fflcommerce_paypal_description',
				'std' => __("Pay via PayPal; you can pay with your credit card if you don't have a PayPal account", 'fflcommerce'),
				'type' => 'longtext'
			),
			array(
				'name' => __('PayPal email address', 'fflcommerce'),
				'desc' => '',
				'tip' => __('Please enter your PayPal email address; this is needed in order to take payment!', 'fflcommerce'),
				'id' => 'fflcommerce_paypal_email',
				'std' => '',
				'type' => 'email'
			),
			array(
				'name' => __('Send shipping details to PayPal', 'fflcommerce'),
				'desc' => '',
				'tip' => __('If your checkout page does not ask for shipping details, or if you do not want to send shipping information to PayPal, set this option to no. If you enable this option PayPal may restrict where things can be sent, and will prevent some orders going through for your protection.', 'fflcommerce'),
				'id' => 'fflcommerce_paypal_send_shipping',
				'std' => 'no',
				'type' => 'checkbox',
				'choices' => array(
					'no' => __('No', 'fflcommerce'),
			...
			array(
				'name' => __('Force payment when free', 'fflcommerce'),
				'desc' => '',
				'tip' => __('If product totals are free and shipping is also free (excluding taxes), this will force 0.01 to allow paypal to process payment. Shop owner is responsible for refunding customer.', 'fflcommerce'),
				'id' => 'fflcommerce_paypal_force_payment',
				'std' => 'no',
				'type' => 'checkbox',
				'choices' => array(
					'no' => __('No', 'fflcommerce'),
			...
			array(
				'name' => __('Enable PayPal sandbox', 'fflcommerce'),
				'desc' => __('Turn on to enable the PayPal sandbox for testing.  Visit <a href="http://developer.paypal.com/">http://developer.paypal.com/</a> for more information and to register a merchant and customer testing account.', 'fflcommerce'),
				'tip' => '',
				'id' => 'fflcommerce_paypal_testmode',
				'std' => 'no',
				'type' => 'checkbox',
				'choices' => array(
					'no' => __('No', 'fflcommerce'),
			...
			array(
				'name' => __('Sandbox email address', 'fflcommerce'),
				'desc' => '',
				'tip' => __('Please enter your Sandbox Merchant email address for use as your sandbox storefront if you have enabled the PayPal sandbox.', 'fflcommerce'),
				'id' => 'fflcommerce_sandbox_email',
				'std' => '',
				'type' => 'midtext'
			),
		);
