...
add_filter( 'fflcommerce_payment_gateways', 'add_futurepay_gateway', 5 );


class futurepay extends fflcommerce_payment_gateway {
	...
	public function __construct() {
		...
		self::$request_url  =
			FFLCommerce_Base::get_options()->get('fflcommerce_futurepay_mode') == 'no'
	...
	protected function get_default_options() {
		...
		$defaults[] = array(
			'name'		=> __('Enable FuturePay','fflcommerce'),
			'desc' 		=> '',
			'tip' 		=> '',
			'id' 		=> 'fflcommerce_futurepay_enabled',
			'std' 		=> 'no',
			'type' 		=> 'checkbox',
			'choices'	=> array(
		...
		$defaults[] = array(
			...
			'id' 		=> 'fflcommerce_futurepay_title',
			'std' 		=> __('FuturePay','fflcommerce'),
			'type' 		=> 'text'
		);
		...
		$defaults[] = array(
			...
			'desc' 		=> '',
			'tip' 		=> __('This controls the description which the user sees during checkout.','fflcommerce'),
			'id' 		=> 'fflcommerce_futurepay_description',
			'std' 		=> __('Pay with FuturePay. Buy now and pay later. No credit card needed.  You will be asked to enter your FuturePay username and password, or create an account when you Place your Order.', 'fflcommerce'),
			'type' 		=> 'longtext'
		);

		$defaults[] = array(
			'name'		=> __('Merchant API Key','fflcommerce'),
			'desc' 		=> '',
			'tip' 		=> __('Your unique FuturePay Merchant Identifier is provided to you when you create a Merchant Account with FuturePay.','fflcommerce'),
			'id' 		=> 'fflcommerce_futurepay_gmid',
			'std' 		=> '',
			'type' 		=> 'longtext'
		);

		$defaults[] = array(
			'name'		=> __('Enable Sandbox','fflcommerce'),
			'desc' 		=> __('Turn on to enable the FuturePay sandbox for testing.', 'fflcommerce'),
			'tip' 		=> '',
			'id' 		=> 'fflcommerce_futurepay_mode',
			'std' 		=> 'no',
			'type' 		=> 'checkbox',
			'choices'	=> array(
				'no'		=> __('No', 'fflcommerce'),
	...
	public function futurepay_script() {
		if ( ! is_page( fflcommerce_get_page_id( 'checkout' )) ) return;
    	?>
		...
		try {
			...
			if ( ! strstr( $response, 'FPTK' ) ) {
				$error_message = isset( self::$futurepay_errorcodes[$response] )
					...
					: __('An unknown error has occured with code = ', 'fflcommerce') . $response;
	...
	public function process_payment( $order_id ) {
		...
		return array(
			'result' 	=> 'success',
			'redirect'	=> add_query_arg( 'order', $order->id, add_query_arg( 'key', $order->order_key, get_permalink(fflcommerce_get_page_id('pay')) ))
		);
	...
	public function receipt_page( $order ) {
		...
		echo '<p>'.__('Thank you for your order, please fill out the form below to pay with FuturePay.', 'fflcommerce').'</p>';
	...
	public function successful_request( $response, $order ) {
		...
		switch ( strtoupper( $response['OrderStatusCode'] ) ) {
			case 'ACCEPTED':
				$order->add_order_note( __('Payment Authorized', 'fflcommerce') );
				fflcommerce_log( "FuturePay: payment authorized for Order ID: " . $order->id );
				$order->payment_complete();
				break;

			case 'DECLINED':
				// Hold order
		        $order->update_status( 'on-hold', sprintf(__('Payment %s via FuturePay.', 'fflcommerce'), strtolower( $response['OrderStatusCode'] ) ) );
		        fflcommerce_log( "FUTUREPAY: declined order for Order ID: " . $order->id );
				break;

			default:
				// Hold order
		        $order->update_status( 'on-hold', sprintf(__('Payment %s via FuturePay.', 'fflcommerce'), strtolower( $response['OrderStatusCode'] ) ) );
		        fflcommerce_log( "FUTUREPAY: failed order for Order ID: " . $order->id );
				break;
		}
	...
	private static $futurepay_errorcodes = array(
		...
		=> 'Invalid Customer Status, the customer exists in FuturePay and is in an Active or Accepted Status', 'fflcommerce',
		'FP_INVALID_ID_REQUEST'
		=> 'Error: The GMID could not be validated – either missing or not valid format – Contact FuturePay', 'fflcommerce',
		'FP_INVALID_SERVER_REQUEST'
		=> 'Error: Either the Merchant Server is not on our IP Whitelist or the Order Reference was Missing', 'fflcommerce',
		'FP_PRE_ORDER_EXCEEDS_MAXIMUM'
		=> 'The Maximum Amount for a FuturePay order has been exceeded: Currently $500.00', 'fflcommerce',
		'FP_MISSING_REFERENCE' =>
		'Reference was not detected in the Query String', 'fflcommerce',
		'FP_INVALID_REFERENCE'
		=> 'Reference was invalid', 'fflcommerce',
		'FP_ORDER_EXISTS'
		=> 'The reference exists with an order that has completed sales attached', 'fflcommerce',
		'FP_MISSING_REQUIRED_FIRST_NAME'
		=> 'First Name was not detected in the Query String', 'fflcommerce',
		'FP_MISSING_REQUIRED_LAST_NAME'
		=> 'Last Name was not detected in the Query String', 'fflcommerce',
		'FP_MISSING_REQUIRED_PHONE'
		=> 'Phone Name was not detected in the Query String', 'fflcommerce',
		'FP_MISSING_REQUIRED_CITY'
		=> 'City was not detected in the Query String', 'fflcommerce',
		'FP_MISSING_REQUIRED_STATE'
		=> 'State was not detected in the Query String', 'fflcommerce',
		'FP_MISSING_REQUIRED_ADDRESS'
		=> 'Address was not detected in the Query String', 'fflcommerce',
		'FP_MISSING_REQUIRED_COUNTRY'
		=> 'Country was not detected in the Query String', 'fflcommerce',
		'FP_COUNTRY_US_ONLY'
		=> 'The Country was not USA', 'fflcommerce',
		'FP_MISSING_EMAIL'
		=> 'Email was not detected in the Query String', 'fflcommerce',
		'FP_INVALID_EMAIL_SIZE'
		=> 'Email Size was greater than 85', 'fflcommerce',
		'FP_INVALID_EMAIL_FORMAT'
		=> 'Email Format was not valid', 'fflcommerce',
		'FP_MISSING_REQUIRED_ZIP'
		=> 'Zip was not detected in the Query String', 'fflcommerce',
		'FP_NO_ZIP_FOUND'
		=> 'The Zip Code could not be found in the FuturePay lookup, may be a PO Box or Military Address which are not Accepted', 'fflcommerce',
		'FP_FAILED_ZIP_LOOKUP'
		=> 'FuturePay failed to lookup the Zip Code – FuturePay needs to investigate the cause', 'fflcommerce',
		'FP_MISSING_ORDER_ITEM_FIELDS'
		=> 'At least one order item must exist and for each order item all of the fields must exist for price, quantity, sku, description, tax_amount', 'fflcommerce',
		'FP_INVALID_PRICE'
		=> 'Price must be a non-negative float value', 'fflcommerce',
		'FP_INVALID_TAX'
		=> 'Tax must be a non-negative float value', 'fflcommerce',
		'FP_INVALID_QUANTITY'
		=> 'Quantity must be an integer', 'fflcommerce',
		'FP_INVALID_SHIPPING_DATE'
		=> 'The Shipping date could not be parsed', 'fflcommerce',
		'FP_SHIPPING_IN_PAST'
		=> 'The Shipping date must be today or in the Future', 'fflcommerce',
		'FP_PRE_ORDER_FAILED'
		=> 'An Error occurred in trying to save the Order – FuturePay needs to investigate the cause', 'fflcommerce'
	);
