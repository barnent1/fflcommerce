 ...
 */
class FFLCommerce_Widget_Cart extends WP_Widget
{
	private $options;
	...
	{
		$options = array(
			'classname' => 'fflcommerce_cart',
			'description' => __('Shopping Cart for the sidebar', 'fflcommerce')
		);
	...
	public function widget($args, $instance)
	{
		...
		$title = apply_filters(
			'widget_title',
			($instance['title']) ? $instance['title'] : __('Cart', 'fflcommerce'),
			$instance,
			$this->id_base
		);
	...
	{
		$total = 0;
		if (!empty(fflcommerce_cart::$cart_contents)) {
	...
	public function form($instance)
	{
		...
		echo "
		<p>
			<label for='{$this->get_field_id('view_cart_button')}'>".__('View cart button:', 'fflcommerce')."</label>
		...
		echo "
		<p>
			<label for='{$this->get_field_id('checkout_button')}'>".__('Checkout button:', 'fflcommerce')."</label>
