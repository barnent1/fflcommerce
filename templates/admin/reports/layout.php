...
<div class="wrap fflcommerce">
	<div class="icon32 icon32-fflcommerce-reports" id="icon-fflcommerce"><br /></div>
	<h2 class="nav-tab-wrapper">
		<?php
		foreach ($reports as $key => $report_group) {
			echo '<a href="'.admin_url('admin.php?page=fflcommerce_reports&tab='.urlencode($key)).'" class="nav-tab ';
		...
		?>
		<?php do_action('fflcommerce_reports_tabs'); ?>
	</h2>
		...
		<ul class="subsubsub">
			<li><?php
				...
				foreach ($reports[$current_tab]['reports'] as $key => $report) {
					$link = '<a href="admin.php?page=fflcommerce_reports&tab='.urlencode($current_tab).'&amp;report='.urlencode($key).'" class="';
