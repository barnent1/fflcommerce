...
<form id="pay_for_order" method="post">
        ...
        <tfoot>
            <tr>
                <?php if (($options->get('fflcommerce_calc_taxes') == 'yes' && $order->has_compound_tax())
                    ...
                    <td colspan="2"><?php _e('Retail Price', 'fflcommerce'); ?></td>
                <?php else : ?>
            ...
            if ($order->order_shipping > 0) :
                ?><tr>
                    <td colspan="2"><?php _e('Shipping', 'fflcommerce'); ?></td>
                    <td><?php echo $order->get_shipping_to_display(); ?></small></td>
                </tr><?php
            endif;
            if ($options->get('fflcommerce_tax_after_coupon') == 'yes' && $order->order_discount > 0) : ?>
                <tr class="discount">
            ...
            if (($options->get('fflcommerce_calc_taxes') == 'yes' && $order->has_compound_tax())
              || ($options->get('fflcommerce_tax_after_coupon') == 'yes' && $order->order_discount > 0)) :
                ?><tr>
                    <td colspan="2"><?php _e('Subtotal', 'fflcommerce'); ?></td>
            ...
            if ($options->get('fflcommerce_tax_after_coupon') == 'no' && $order->order_discount > 0) : ?><tr class="discount">
                    <td colspan="2"><?php _e('Discount', 'fflcommerce'); ?></td>
                    <td>-<?php echo fflcommerce_price($order->order_discount); ?></td>
                </tr><?php endif; ?>
            <tr>
    ...
    <div id="payment">
        ...
        <div class="form-row">
            <?php fflcommerce::nonce_field('pay') ?>
            <input type="submit" class="button-alt" name="pay" id="place_order" value="<?php _e('Pay for order', 'fflcommerce'); ?>" />
