<div id="payment">
	...
	<div class="form-row">
		<noscript>
			<?php _e('Since your browser does not support JavaScript, or it is disabled, please ensure you click the <em>Update Totals</em> button before placing your order. You may be charged more than the amount stated above if you fail to do so.', 'fflcommerce'); ?>
			<br /><input type="submit" class="button-alt" name="update_totals" value="<?php _e('Update totals', 'fflcommerce'); ?>" />
		</noscript>

		<?php fflcommerce::nonce_field('process_checkout') ?>

		<?php do_action('fflcommerce_review_order_before_submit'); ?>

		<?php if (fflcommerce_get_page_id('terms') > 0) : ?>
			<p class="form-row terms">
				<label for="terms" class="checkbox"><?php _e('I accept the', 'fflcommerce'); ?>
					<a href="<?php echo esc_url(get_permalink(fflcommerce_get_page_id('terms'))); ?>" target="_blank"><?php _e('terms &amp; conditions', 'fflcommerce'); ?></a>
				</label>
				<input type="checkbox" class="input-checkbox" name="terms" id="terms"<?php isset($_POST['terms']) and print ' checked="checked"'; ?> />
			</p>
		<?php endif; ?>

		<?php $order_button_text = apply_filters('fflcommerce_order_button_text', __('Place Order', 'fflcommerce')); ?>
		<input type="submit" class="button-alt" name="place_order" id="place_order" value="<?php echo esc_attr($order_button_text); ?>" />

		<?php do_action('fflcommerce_review_order_after_submit'); ?>
	</div>
</div>