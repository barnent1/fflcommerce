...
<ul class="products <?php echo $orientation; ?> <?php echo $has_thumbnails ? 'thumbnails' : ''; ?>">
	...
	<li>
		<?php $product = new fflcommerce_product($post->ID); ?>
		<div class="before-product">
			<?php do_action('fflcommerce_before_shop_loop_item', $post, $product); ?>
		</div>
		<a class="product" href="<?php echo $product->get_link(); ?>">
			<?php do_action('fflcommerce_before_shop_loop_item_title', $post, $product); ?>
			<strong><?php echo $product->get_title(); ?></strong>
			<?php do_action('fflcommerce_after_shop_loop_item_title', $post, $product); ?>
		</a>
		<div class="after-product">
			<?php do_action('fflcommerce_after_shop_loop_item', $post, $product); ?>
		</div>
	</li>
	<?php endforeach; ?>
</ul>