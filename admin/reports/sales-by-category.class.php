...
class FFLCommerce_Report_Sales_By_Category extends FFLCommerce_Admin_Report
{
	...
	public function get_chart_legend()
	{
		...
		foreach ($this->show_categories as $category) {
			...
			$legend[] = array(
				'title' => sprintf(__('%s sales in %s', 'fflcommerce'), '<strong>'.fflcommerce_price($total).'</strong>', $category->name),
	...
	public function get_products_in_category($category_id)
	{
		...
		return array_unique(apply_filters('fflcommerce_report_sales_by_category_get_products_in_category', $product_ids, $category_id));
	}
	...
	public function get_chart_widgets()
	{
		return array(
			array(
				'title' => __('Categories', 'fflcommerce'),
	...
	public function category_widget()
	{
		...
		<form method="GET">
			<div>
				<select multiple="multiple" data-placeholder="<?php _e('Select categories&hellip;', 'fflcommerce'); ?>" class="fflcommerce-enhanced-select" id="show_categories" name="show_categories[]" style="width: 205px;">
					<?php
					$r = array();
					$r['pad_counts'] = 1;
					$r['hierarchical'] = 1;
					...
					echo fflcommerce_walk_category_dropdown_tree($categories, 0, $r);
					?>
				</select>
				<a href="#" class="select_none"><?php _e('None', 'fflcommerce'); ?></a>
				<a href="#" class="select_all"><?php _e('All', 'fflcommerce'); ?></a>
	...
	public function get_main_chart()
	{
		...
		if (!$this->show_categories) {
			...
			<div class="chart-container">
				<p class="chart-prompt"><?php _e( '&larr; Choose a category to view stats', 'fflcommerce' ); ?></p>
			</div>
			<?php
		} else {
			$chart_data = array();
