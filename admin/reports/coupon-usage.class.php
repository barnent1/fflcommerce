...
class FFLCommerce_Report_Coupon_Usage extends FFLCommerce_Admin_Report
{
	...
	public function get_chart_legend()
	{
		...
		$legend[] = array(
			'title' => sprintf(__('%s discounts in total', 'fflcommerce'), '<strong>'.fflcommerce_price($total_discount).'</strong>'),
			'color' => $this->chart_colours['discount_amount'],
			'highlight_series' => 1
		);

		$legend[] = array(
			'title' => sprintf(__('%s coupons used in total', 'fflcommerce'), '<strong>'.$total_coupons.'</strong>'),
	...
	{
		?>
		<h4 class="section_title"><span><?php _e('Filter by coupon', 'fflcommerce'); ?></span></h4>
		<div class="section">
			<form method="GET">
				<div>
					<?php
					...
					if ($used_coupons) :
					?>
						<select id="coupon_codes" name="coupon_codes" class="wc-enhanced-select" data-placeholder="<?php _e('Choose coupons&hellip;', 'fflcommerce'); ?>" style="width:100%;">
							<option value=""><?php _e('All coupons', 'fflcommerce'); ?></option>
							<?php
					...
					<?php else : ?>
						<span><?php _e('No used coupons found', 'fflcommerce'); ?></span>
					<?php endif; ?>
				</div>
			</form>
		</div>
		<h4 class="section_title"><span><?php _e('Most Popular', 'fflcommerce'); ?></span></h4>
		<div class="section">
			<table cellspacing="0">
				<?php
		...
		<h4 class="section_title"><span><?php _e('Most Discount', 'fflcommerce'); ?></span></h4>
		<div class="section">
			<table cellspacing="0">
				<?php
		...
		<script type="text/javascript">
			...
			jQuery(function($){
				...
				var drawGraph = function(highlight){
					var series = [
						{
							label: "<?php echo esc_js(__('Number of coupons used', 'fflcommerce')) ?>",
						...
						{
							label: "<?php echo esc_js(__('Discount amount', 'fflcommerce')) ?>",
