...
if (!class_exists('FFLCommerce_Report_Stock')) {
	require_once('stock.class.php');
}

class FFLCommerce_Report_Low_In_Stock extends FFLCommerce_Report_Stock
{
	public function no_items()
	{
		_e('No low in stock products found.', 'fflcommerce');
	}
	...
	public function get_items($current_page, $per_page)
	{
		...
		$stock = absint(max($options->get('fflcommerce_notify_low_stock_amount'), 1));
		$nostock = absint(max($options->get('fflcommerce_notify_no_stock_amount'), 0));

		$query_from = apply_filters('fflcommerce_report_low_in_stock_query_from', "FROM {$wpdb->posts} as posts
