...
abstract class FFLCommerce_Admin_Report
{
	...
	public function get_order_report_data($args = array())
	{
		...
		$args = apply_filters('fflcommerce_reports_get_order_report_data_args', $args);
		...
		$order_status = apply_filters('fflcommerce_reports_order_statuses', $args['order_status']);
		...
		$query = apply_filters('fflcommerce_reports_get_order_report_query', $query);
		$query = implode(' ', $query);
		...
		if ($args['debug'] || $args['nocache'] || false === $cached_results || !isset($cached_results[$query_hash])) {
			$cached_results[$query_hash] = apply_filters('fflcommerce_reports_get_order_report_data', $wpdb->{$args['query_type']}($query), $args['data']);
