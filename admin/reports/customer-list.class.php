...
class FFLCommerce_Report_Customer_List extends WP_List_Table
{
	...
	public function __construct()
	{
		parent::__construct(array(
			'singular' => __('Customer', 'fflcommerce'),
			'plural' => __('Customers', 'fflcommerce'),
			'ajax' => false
		));
	}
	...
	{
		_e('No customers found.', 'fflcommerce');
	}
	...
	public function output()
	{
		...
		echo '<div id="poststuff" class="fflcommerce-reports-wide">';
		if (!empty($_GET['link_orders']) && wp_verify_nonce($_REQUEST['_wpnonce'], 'link_orders')) {
			...
			echo '<div class="updated"><p>'.sprintf(_n('%s previous order linked', '%s previous orders linked', $linked, 'fflcommerce'), $linked).'</p></div>';
		}
		...
		if (!empty($_GET['refresh']) && wp_verify_nonce($_REQUEST['_wpnonce'], 'refresh')) {
			...
			echo '<div class="updated"><p>'.sprintf(__('Refreshed stats for %s', 'fflcommerce'), $user->display_name).'</p></div>';
		}

		echo '<form method="post" id="fflcommerce_customers">';

		$this->search_box(__('Search customers', 'fflcommerce'), 'customer_search');
		$this->display();
	...
	{
		$columns = array(
			'customer_name' => __('Name (Last, First)', 'fflcommerce'),
			'username' => __('Username', 'fflcommerce'),
			'email' => __('Email', 'fflcommerce'),
			'location' => __('Location', 'fflcommerce'),
			'orders' => __('Orders', 'fflcommerce'),
			'spent' => __('Money Spent', 'fflcommerce'),
			'last_order' => __('Last order', 'fflcommerce'),
			'user_actions' => __('Actions', 'fflcommerce')
		);
		...
		switch ($column_name) {
			...
			case 'user_actions' :
				...
				$actions['refresh'] = array(
					...
					'name' => __('Refresh stats', 'fflcommerce'),
					'action' => 'refresh'
				);

				$actions['edit'] = array(
					'url' => admin_url('user-edit.php?user_id='.$user->ID),
					'name' => __('Edit', 'fflcommerce'),
					'action' => 'edit'
				);
				...
				if ($order_ids) {
					$actions['link'] = array(
						...
						'name' => __('Link previous orders', 'fflcommerce'),
						'action' => 'link'
					);
				}
