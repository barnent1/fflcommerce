...
class FFLCommerce_Report_Sales_By_Date extends FFLCommerce_Admin_Report
{
	...
	public function get_chart_legend()
	{
		...
		/** @noinspection PhpUndefinedFieldInspection */
		$legend[] = array(
			'title' => sprintf(__('%s gross sales in this period', 'fflcommerce'), '<strong>'.fflcommerce_price($data->total_sales).'</strong>'),
			'placeholder' => __('This is the sum of the order totals including shipping and taxes.', 'fflcommerce'),
			'color' => $this->chart_colours['sales_amount'],
			'highlight_series' => 5
		);
		/** @noinspection PhpUndefinedFieldInspection */
		$legend[] = array(
			'title' => sprintf(__('%s net sales in this period', 'fflcommerce'), '<strong>'.fflcommerce_price($data->net_sales).'</strong>'),
			'placeholder' => __('This is the sum of the order totals excluding shipping and taxes.', 'fflcommerce'),
		...
		/** @noinspection PhpUndefinedFieldInspection */
		$legend[] = array(
			'title' => sprintf(__('%s orders placed', 'fflcommerce'), '<strong>'.$data->total_orders.'</strong>'),
		...
		/** @noinspection PhpUndefinedFieldInspection */
		$legend[] = array(
			'title' => sprintf(__('%s items purchased', 'fflcommerce'), '<strong>'.$data->total_items.'</strong>'),
		...
		/** @noinspection PhpUndefinedFieldInspection */
		$legend[] = array(
			'title' => sprintf(__('%s charged for shipping', 'fflcommerce'), '<strong>'.fflcommerce_price($data->total_shipping).'</strong>'),
			'color' => $this->chart_colours['shipping_amount'],
			'highlight_series' => 4
		);
		/** @noinspection PhpUndefinedFieldInspection */
		$legend[] = array(
			'title' => sprintf(__('%s worth of coupons used', 'fflcommerce'), '<strong>'.fflcommerce_price($data->total_coupons).'</strong>'),
	...
	private function query_report_data()
	{
		...
		$this->report_data->total_sales = fflcommerce_format_decimal(array_sum(wp_list_pluck($this->report_data->orders, 'total_sales')), 2);
		$this->report_data->total_tax = fflcommerce_format_decimal(array_sum(wp_list_pluck($this->report_data->orders, 'total_tax')), 2);
		$this->report_data->total_shipping = fflcommerce_format_decimal(array_sum(wp_list_pluck($this->report_data->orders, 'total_shipping')), 2);
		$this->report_data->total_shipping_tax = fflcommerce_format_decimal(array_sum(wp_list_pluck($this->report_data->orders, 'total_shipping_tax')), 2);
		...
		$this->report_data->average_sales = fflcommerce_format_decimal($this->report_data->total_sales / ($this->chart_interval + 1), 2);
		$this->report_data->net_sales = fflcommerce_format_decimal($this->report_data->total_sales - $this->report_data->total_shipping - $this->report_data->total_tax - $this->report_data->total_shipping_tax, 2);
	}
	...
	public function output()
	{
		...
		$ranges = array(
			'all' => __('All Time', 'fflcommerce'),
			'year' => __('Year', 'fflcommerce'),
			'last_month' => __('Last Month', 'fflcommerce'),
			'month' => __('This Month', 'fflcommerce'),
			'30day' => __('Last 30 Days', 'fflcommerce'),
			'7day' => __('Last 7 Days', 'fflcommerce'),
			'today' => __('Today', 'fflcommerce'),
		);
	...
	public function get_chart_widgets()
	{
		...
		$widgets[] = array(
			'title' => __('Order status filter', 'fflcommerce'),
	...
	public function order_status_widget()
	{
		...
		<form method="GET">
			...
			$args = array(
				...
				'placeholder' => '',
				'options' => array(
					'pending' => __('Pending', 'fflcommerce'),
					'on-hold' => __('On-Hold', 'fflcommerce'),
					'waiting-for-payment' => __('Waiting for payment', 'fflcommerce'),
					'processing' => __('Processing', 'fflcommerce'),
					'completed' => __('Completed', 'fflcommerce'),
					'cancelled' => __('Cancelled', 'fflcommerce'),
					'refunded' => __('Refunded', 'fflcommerce')
				),
				'selected' => $this->order_status,
			);
			echo fflcommerce_Forms::select($args);
			?>
			<input type="submit" class="submit button" value="<?php _e('Show', 'fflcommerce'); ?>" />
	...
	public function get_export_button()
	{
		...
		?>
		<a href="#" download="report-<?php echo esc_attr($current_range); ?>-<?php echo date_i18n('Y-m-d', current_time('timestamp')); ?>.csv" class="export_csv" data-export="chart" data-xaxes="<?php _e('Date', 'fflcommerce'); ?>" data-exclude_series="2" data-groupby="<?php echo $this->chart_groupby; ?>">
			<?php _e('Export CSV', 'fflcommerce'); ?>
		</a>
	<?php
	}
		...
		<script type="text/javascript">
			...
			jQuery(function(){
				...
				var drawGraph = function(highlight){
					var series = [
						{
							label: "<?php echo esc_js( __( 'Number of items sold', 'fflcommerce' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Number of orders', 'fflcommerce' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Average sales amount', 'fflcommerce' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Coupon amount', 'fflcommerce' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Shipping amount', 'fflcommerce' ) ) ?>",
							...
							lines: {show: true, lineWidth: 2, fill: false},
							shadowSize: 0,
							prepend_tooltip: "<?php echo get_fflcommerce_currency_symbol(); ?>"
						},
						{
							label: "<?php echo esc_js( __( 'Gross Sales amount', 'fflcommerce' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Net Sales amount', 'fflcommerce' ) ) ?>",
	...
	private function round_chart_totals($amount)
	{
		if (is_array($amount)) {
			...
			return fflcommerce_format_decimal($amount, '');
		}
	}
}
