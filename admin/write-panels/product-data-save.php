 ...
 * 
 */
class fflcommerce_product_meta
{
	public function __construct() {
		add_action( 'fflcommerce_process_product_meta', array(&$this, 'save'), 1, 2 );
	}
	...
	public function save($post_id)
	{
		...
		$sale_price = !empty($_POST['sale_price'])
			? (!strstr($_POST['sale_price'], '%') ? fflcommerce_sanitize_num($_POST['sale_price']) : $_POST['sale_price'])
			: '';
	...
	private function process_stock( array $post ) {
		...
		if ( !$fflcommerce_options->get('fflcommerce_manage_stock') )
			return false;
		...
		if( $array['manage_stock'] ) {
			...
			if ( $fflcommerce_options->get( 'fflcommerce_hide_no_stock_product' ) == 'yes' ) {
				if ( $array['stock'] <= $fflcommerce_options->get( 'fflcommerce_notify_no_stock_amount' ) ) {
	...
	private function is_unique_sku( $post_id, $new_sku ) {
		...
		if ( $wpdb->get_var($_unique_meta) || $wpdb->get_var($_unique_post_id) )
			return new WP_Error( 'fflcommerce_unique_sku', __('Product SKU must be unique', 'fflcommerce') );
