 ...
 */
add_action( 'admin_print_scripts-edit.php', 'fflcommerce_enqueue_product_quick_scripts' );
...
function fflcommerce_enqueue_product_quick_scripts() {
	...
	if ( $typenow == 'product' ) {
		...
		$fflcommerce_quick_edit_params = array(
			'assets_url' 				=> fflcommerce::assets_url(),
 ...
 */
add_action( 'wp_ajax_fflcommerce_get_product_stock_price', 'fflcommerce_ajax_get_product_stock_price' );

function fflcommerce_ajax_get_product_stock_price() {

	check_ajax_referer( 'get-product-stock-price', 'security' );

	$values = array();

	$_product = new fflcommerce_product( $_GET['post_id'] );

	if ( $_product->managing_stock() ) {
		$values['stock'] = get_post_meta( $_GET['post_id'], 'stock', true );
	} else {
		$values['stock'] = __('Not managed','fflcommerce');
	}
	if ( $_product->is_type( array( 'grouped' ) ) ) {
		$values['price'] = __('Grouped parent has no price','fflcommerce');
	} else {
 ...
 */
add_action( 'bulk_edit_custom_box', 'fflcommerce_add_to_bulk_quick_edit_custom_box', 10, 2 );
add_action( 'quick_edit_custom_box', 'fflcommerce_add_to_bulk_quick_edit_custom_box', 10, 2 );

function fflcommerce_add_to_bulk_quick_edit_custom_box( $column_name, $post_type ) {
	...
	case 'product':
		...
		case 'stock':
			...
			<fieldset class="inline-edit-col-right">
				<div class="inline-edit-col">
					<label><span class="title"><?php _e( 'Stock', 'fflcommerce' ); ?></span>
		...
		case 'price':
			...
			<fieldset class="inline-edit-col-right">
				<div class="inline-edit-col">
					<label><span class="title"><?php _e( 'Price', 'fflcommerce' ); ?></span>
 ...
 */
add_action( 'save_post','fflcommerce_save_quick_edit', 10, 2 );

function fflcommerce_save_quick_edit($post_id, $post)
{
	...
	switch ($post->post_type) {
		case 'product':
			...
			if (array_key_exists('stock', $_POST) && $_product->managing_stock()) {
				$stock = empty($_POST['stock']) ? 0 : fflcommerce_sanitize_num($_POST['stock']);
			...
			if (array_key_exists('price', $_POST) && !empty($_POST['price'])) {
				if (!$_product->is_type(array('grouped'))) {
					if ($_POST['price'] == null) {
						...
						update_post_meta($post_id, 'regular_price', fflcommerce_sanitize_num($_POST['price']));
					}
				}
			}
			break;
 ...
 */
add_action( 'wp_ajax_fflcommerce_save_bulk_edit', 'fflcommerce_save_bulk_edit' );
